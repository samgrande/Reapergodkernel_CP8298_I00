...
#include "SynaImage_ofilm_alt.h"
#include "SynaImage_yeji_alt.h"
#include <mach/mt_boot.h>//open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150811


#define FW_IMAGE_NAME "synaptics/startup_fw_update.img"

#define DO_STARTUP_FW_UPDATE //open by jingyuantao for TW auto update in 20150805
...
DECLARE_COMPLETION(fwu_remove_complete_alt);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		dev_dbg(&fwu->rmi4_data->i2c_client->dev,
				...
				__func__, fwu->image_size);//fw_entry->size);
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150811*/
u8 product_id_alt[11];
u8  *get_RMI_product_id_alt(void)
{
    int i = 0;
    memset(product_id_alt,0,sizeof(product_id_alt));
    fwu->fn_ptr->read(fwu->rmi4_data,fwu->rmi4_data->f01_query_base_addr+11,&product_id_alt[0], 10);
    product_id_alt[10]=0;
    printk("SynaF01QueryBase+11=%x\n",fwu->rmi4_data->f01_query_base_addr+11);
    printk("get_RMI_product_id_alt product_id_alt=%s\n",product_id_alt);

    for( i = 0;i < 11 ; i ++)
        printk("get_RMI_product_id_alt product_id_alt[%d]=%x \n",i,product_id_alt[i]);

    return product_id_alt;
}
#if 0
int synaptics_fw_updater_alt(const unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_fw_updater_alt);
...
int synaptics_fw_updater_alt(int Product_ID)
{
    int retval;
    ...
    if(Product_ID==1)
    {
        ...
        fwu->ext_data_source = SynaImage_yeji_alt;
        fwu->config_area = UI_CONFIG_AREA;
    ...
    if(Product_ID==2)
    {
        printk("SynaImage_ofilm Product_ID==2\n");
        fwu->ext_data_source = SynaImage_ofilm_alt;
        fwu->config_area = UI_CONFIG_AREA;
        fwu->image = fwu->ext_data_source;
        retval = fwu_start_reflash();
    }
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
    ...
    if(product_id_alt[8]==0x61&&product_id_alt[9]==0x31)//yeji 0xa1
        synaptics_fw_updater_alt(1);
    else if(product_id_alt[8]==0x61&&product_id_alt[9]==0x30)//ofilm 0xa0
        synaptics_fw_updater_alt(2);

    return;
}
#endif

unsigned char *yl_firmware_alt[3] = {
    "yeji",
    "ofilm",
    "NA"
};
...
int synaptics_tp_get_dev_info_alt()
{
        ...
        if(product_id_alt[8]==0x61&&product_id_alt[9]==0x31)//yeji 0xa1
            sprintf(buf, "touchkey: %s:syna1302:0x%02x:0x%02x\n",yl_firmware_alt[0], config_id[1], config_id[3]);
        else if(product_id_alt[8]==0x61&&product_id_alt[9]==0x30)//ofilm 0xa0
            sprintf(buf, "touchkey: %s:syna1302:0x%02x:0x%02x\n",yl_firmware_alt[1], config_id[1], config_id[3]);
        else
            sprintf(buf, "touchkey: %s:syna1302:0x%02x:0x%02x\n",yl_firmware_alt[2], config_id[1], config_id[3]);
...
int synaptics_tp_get_version_alt(unsigned char *buf)
{
        unsigned char config_id[4] = {0};
        if (!fwu)
        {
...
int synaptics_fw_needupdater_alt(void)
{
        unsigned char config_id[4]={0};
       ...
       printk("SynaImage_yeji[0xb100+3]=%x,SynaImage_ofilm[0xb100+3]=%x,config_id[1]=%x,config_id[3]=%x\n",SynaImage_yeji_alt[0xb100+3],SynaImage_ofilm_alt[0xb100+3],config_id[1],config_id[3]);
       
        if(product_id_alt[8]==0x61&&product_id_alt[9]==0x31)//yeji:0xa1
        {
             if(SynaImage_yeji_alt[0xb100+3]>config_id[3])
                 return 1;
             else return 0;
             
        }
        if(product_id_alt[8]==0x61&&product_id_alt[9]==0x30)//ofilm 0xa0
        {
             if(SynaImage_ofilm_alt[0xb100+3]>config_id[3])
                 return 1;
             else return 0;
        }
...
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150811*/
static ssize_t fwu_sysfs_show_image(struct file *data_file,
		struct kobject *kobj, struct bin_attribute *attributes,
		char *buf, loff_t pos, size_t count)
{
...
/*start to open update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    //retval = synaptics_fw_updater_alt(fwu->image);
    get_RMI_product_id_alt();
    if(product_id_alt[8]==0x61&&product_id_alt[9]==0x31)//yeji 0xa1
        retval = synaptics_fw_updater_alt(1);
    else if(product_id_alt[8]==0x61&&product_id_alt[9]==0x30)//ofilm 0xa0
        retval = synaptics_fw_updater_alt(2);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    int boot_mode = 0; /*open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150811*/
...
exit:
	complete(&fwu_remove_complete_alt);
...
{
	synaptics_rmi4_new_function_alt(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_alt(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete_alt);
