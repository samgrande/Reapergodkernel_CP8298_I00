...
#include "tpd.h"
/*start: add .h file by fanli@yulong.com to resolve compile problem in 2014.09.27*/
#include  "cust_eint_md1.h" 
#include "touchscreen.h"   
/*end:  add .h file by fanli@yulong.com to resolve compile problem in 2014.09.27*/
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
extern u8 product_id_info[11];
extern u8  *get_RMI_product_id(void);
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
#define TPD_SMART_HULL 1

#if TPD_SMART_HULL
#define HULL_STATE_OPEN    0
#define HULL_STATE_CLOSE   1
#define SMART_HULL_SWITCH_OPEN 1
#define SMART_HULL_SWITCH_CLOSE 0

static int hall_state = HULL_STATE_OPEN;        //0,open 1,close
static int smarthull_switch = SMART_HULL_SWITCH_CLOSE;  //1,open 0,close in setting
touch_mode_type tp_mode_bak = 0;
#endif

unsigned char glove_value = 0;
unsigned int glove_input = 0;
...
#define DELAY_HARDWARE_RESET       15 //modify for reset TP when resume by jingyuantao@yulong.com in 20150928
#define DELAY_HARDWARE_RESET_READY 150 //modify for reset TP when resume by jingyuantao@yulong.com in 20150928
...
#define SYNAPTICS_SURRPORT_DOUBLE_TAP     0x01//add gesture mode by jingyuantao@yulong.com in 20150827
#define SYNAPTICS_SURRPORT_UNICODE_CHAR   0x40//add gesture mode by jingyuantao@yulong.com in 20150827
#define SYNAPTICS_SURRPORT_SWIPE          0x07
#define SYNAPTICS_SURRPORT_CIRCLE         0x08

#define SYNAPTICS_SURRPORT_SWIPE_UP       0x08
#define SYNAPTICS_SURRPORT_SWIPE_DOWN     0x04
#define SYNAPTICS_SURRPORT_SWIPE_LEFT     0x02
#define SYNAPTICS_SURRPORT_SWIPE_RIGHT    0x01

#define SYNAPTICS_SURRPORT_UNICODE_C      0x63
#define SYNAPTICS_SURRPORT_UNICODE_W      0x77
#define SYNAPTICS_SURRPORT_UNICODE_M      0x6D
#define SYNAPTICS_SURRPORT_UNICODE_E      0x65

char *click_env[2]={"GESTURE=DOUBLE_CLICK",NULL};
char *c_env[2]={"GESTURE=C",NULL};
char *e_env[2]={"GESTURE=E",NULL};
char *w_env[2]={"GESTURE=W",NULL};
char *m_env[2]={"GESTURE=M",NULL};
char *o_env[2]={"GESTURE=O",NULL};
char *up_env[2]={"GESTURE=UP",NULL};
char *down_env[2]={"GESTURE=DOWN",NULL};
char *left_env[2]={"GESTURE=LEFT",NULL};
char *right_env[2]={"GESTURE=RIGHT",NULL};
...
int synaptics_rmi4_clear_irq(struct synaptics_rmi4_data *rmi4_data);
static void synaptics_rmi4_hardware_reset_tp(void);
static  int synaptics_rmi4_config_reg_custom(struct synaptics_rmi4_data *rmi4_data);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	gprDMABuf_va = (u8 *)dma_alloc_coherent(&rmi4_data->input_dev->dev, 4096, &gprDMABuf_pa, GFP_KERNEL);//qinxinjun
        if(!gprDMABuf_va){
...
exit:
        if(gprDMABuf_va){
                dma_free_coherent(&rmi4_data->input_dev->dev, 4096, gprDMABuf_va, gprDMABuf_pa);//qinxinjun
                gprDMABuf_va = NULL;
                gprDMABuf_pa = NULL;
        }
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	rmi4_data->input_dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
	gpwDMABuf_va = (u8 *)dma_alloc_coherent(&rmi4_data->input_dev->dev, 1024, &gpwDMABuf_pa, GFP_KERNEL);//qinxinjun
...
exit:
	...
	if(gpwDMABuf_va){
                dma_free_coherent(&rmi4_data->input_dev->dev, 1024, gpwDMABuf_va, gpwDMABuf_pa);//qinxinjun
                gpwDMABuf_va = NULL;
                gpwDMABuf_pa = NULL;
        }
	mutex_unlock(&(rmi4_data->rmi4_io_ctrl_mutex));
...
/*start to add gesture mode by jingyuantao@yulong.com in 20150827*/
    unsigned char detected_gestures[5];
    struct device *touchscreen_dev;
    unsigned char direction = 0;
/*end to add gesture mode by jingyuantao@yulong.com in 20150827*/
...
/*start to add gesture mode by jingyuantao@yulong.com in 20150827*/
    ...
    if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
        ...
        if (retval < 0)
            return 0;
/*start modify by jingyuantao@yulong.com to close the print log in 20150910*/
                //dev_err(&rmi4_data->i2c_client->dev,
                        //"\n"
                        //"detected_gestures[0]=0x%x \n"
                        //"detected_gestures[1]=0x%x \n"
                        //"detected_gestures[2]=0x%x \n"
                        //"detected_gestures[3]=0x%x \n"
                        //"detected_gestures[4]=0x%x \n",
                        //detected_gestures[0],
                        //detected_gestures[1],
                        //detected_gestures[2],
                        //detected_gestures[3],
                        //detected_gestures[4]);
/*end modify by jingyuantao@yulong.com to close the print log in 20150910*/
                switch(detected_gestures[0])
                {
                    case  SYNAPTICS_SURRPORT_DOUBLE_TAP:
                        ...
                        input_sync(rmi4_data->input_dev);
                        kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,click_env);
                        rmi4_data->sensor_sleep = false;
                        }
                    break;
                    case SYNAPTICS_SURRPORT_UNICODE_CHAR:
                         //dev_err(&rmi4_data->i2c_client->dev,"gesture_type char  is 0x%x\n",detected_gestures[3]);
                         switch(detected_gestures[3])
                         {
                             case SYNAPTICS_SURRPORT_UNICODE_C:
                             if(support_gesture & TW_SUPPORT_C_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,c_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             case SYNAPTICS_SURRPORT_UNICODE_W:
                             if(support_gesture & TW_SUPPORT_W_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,w_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             case SYNAPTICS_SURRPORT_UNICODE_M:
                             if(support_gesture & TW_SUPPORT_M_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,m_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             case SYNAPTICS_SURRPORT_UNICODE_E:
                             if(support_gesture & TW_SUPPORT_E_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,e_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             default:
                    ...
                    case SYNAPTICS_SURRPORT_SWIPE:
                         ...
                         if (retval < 0)
                             return 0;
                         //dev_err(&rmi4_data->i2c_client->dev,"direction is 0x%x\n",direction);
                         switch(direction)
                         {
                             case SYNAPTICS_SURRPORT_SWIPE_UP:
                             ...
                             input_report_key(rmi4_data->input_dev, KEY_POWER, 0);
                             input_sync(rmi4_data->input_dev);
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,up_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             case SYNAPTICS_SURRPORT_SWIPE_DOWN:
                             ...
                             input_report_key(rmi4_data->input_dev, KEY_POWER, 0);
                             input_sync(rmi4_data->input_dev);
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,down_env);
                             rmi4_data->sensor_sleep = false;
                             }
                             break;
                             case SYNAPTICS_SURRPORT_SWIPE_LEFT:
                             if(support_gesture & TW_SUPPORT_LEFT_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,left_env);
                             }
                             break;
                             case SYNAPTICS_SURRPORT_SWIPE_RIGHT:
                             if(support_gesture & TW_SUPPORT_RIGHT_SLIDE_WAKEUP)
                             {
                             kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,right_env);
                             }
                             break;
                             default:
                    ...
                    break;
                    case SYNAPTICS_SURRPORT_CIRCLE:
                    if(support_gesture & TW_SUPPORT_O_SLIDE_WAKEUP)
                    {
                    kobject_uevent_env(&touchscreen_dev->kobj,KOBJ_CHANGE,o_env);
                    rmi4_data->sensor_sleep = false;
                    }
                    break;
                    default:
		...
		if (retval < 0)
			return 0;
                ...
                switch(detected_gestures[0])
                {
                    ...
                    break;
                    case SYNAPTICS_SURRPORT_UNICODE_CHAR:
                         printk("gesture_type char  is 0x%x\n",detected_gestures[2]);
...
/*start: modified by fanli@yulong.com to resolve compile problem in 2014.09.27*/
...
/*end: modified by fanli@yulong.com to resolve compile problem in 2014.09.27*/
		}
		mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
...
int yulong_synaptics_fw_updater(void)
{
        ...
        tpd_halt=1;
        mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
/*start open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
        //ret = synaptics_fw_updater(NULL);
        get_RMI_product_id();
        synaptics_fw_updater(1);
        if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji 0xa1
            ret = synaptics_fw_updater(1);
        else if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue 0xaf
...
extern int synaptics_tp_get_version(struct device *dev,struct device_attribute *attr, char *buf);
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    get_RMI_product_id();
    if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji 0xa1
        return sprintf(vendor,"%s","synaptics_a1");
    else if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue 0xaf
        return sprintf(vendor,"%s","synaptics_af");
    else
        return sprintf(vendor,"%s","synaptics_unknown");
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
}

touch_mode_type synaptics_get_tp_mode(void)
{
    if(MODE_WINDOW == glove_input)
        return 	MODE_WINDOW;
    else if(MODE_GLOVE == glove_input)
        return MODE_GLOVE;
    else
        return MODE_NORMAL;
}

int synaptic_rmi4_glove_mode_enable(struct synaptics_rmi4_data *rmi4_data,touch_mode_type mode_type)
{
        ...
        if(rmi4_data == NULL)
        {
            printk("synaptic_rmi4_glove_mode_enable function rmi4_data is NULL\n");
            return 0;
        }

        printk("At Function %s glove value is 0x%x , type is %d\n", __func__, value,mode_type);
        ...
        if((MODE_GLOVE == mode_type)||(MODE_NORMAL == mode_type))
        {
#if TPD_SMART_HULL
            if(smarthull_switch)
            {
        ...
        if (retval < 0)
        {
                TPD_DMESG("tpd_i2c_write_data() failed: %d\n", retval);
                return retval;
        }
        return retval;
}
        else if(MODE_WINDOW == mode_type)
        {
#if TPD_SMART_HULL
             if(MODE_GLOVE == tp_mode_bak)
             {
                 retval = synaptics_rmi4_i2c_read(rmi4_data,
                     0x001E,
                     &value,
...
int synaptics_set_tp_mode(touch_mode_type type)
{
        unsigned char value;
        int retval;
        ...
        if(g_dev != NULL)
        {
            rmi4_data = dev_get_drvdata(g_dev);
        }
        else
        {
            printk("tp: g_dev is NULL\n");
            return 0;
        }

        retval = synaptic_rmi4_glove_mode_enable(rmi4_data,type);
        return retval;
}

#if SYNA_GESTURE_WAKEUP
int synaptics_get_wakeup_gesture(char* gesture)
{
	return sprintf(gesture, "%s", (char *)wakeup_string_buffer);
}

int synaptics_gesture_ctrl(const char* gesture_buf)
{
  char *gesture;
	int buf_len;
	char *gesture_p;
...
{
        if(state)
                hall_state=HULL_STATE_CLOSE;
        else
                hall_state=HULL_STATE_OPEN;
        if(smarthull_switch)
        {
            if(hall_state)  //hall state is close and smarthull setting is open
            {
                tp_mode_bak = glove_input;
                synaptics_set_tp_mode(MODE_WINDOW);
            }
            else
            {
                glove_input = tp_mode_bak;
                if(MODE_GLOVE == glove_input)
                    synaptics_set_tp_mode(MODE_GLOVE); //set normal config
                else
                    synaptics_set_tp_mode(MODE_NORMAL);
            }
       }

}
EXPORT_SYMBOL(hall_state_notify_touch);
int synaptics_smarthull_ctrl(int setting_switch)
{
...
touchscreen_ops_tpye synaptics_tp_ops = {
        ...
        .firmware_need_update = synaptics_fw_needupdater,
        ...
        .get_mode = synaptics_get_tp_mode,
        .set_mode = synaptics_set_tp_mode,
#if SYNA_GESTURE_WAKEUP
        .get_wakeup_gesture = synaptics_get_wakeup_gesture,
        .gesture_ctrl = synaptics_gesture_ctrl,
#endif
        .smarthull_ctrl = synaptics_smarthull_ctrl,
};
...
#endif
{
    ...
    retval = synaptics_rmi4_clear_irq(rmi4_data);
    if (retval < 0) {
        printk("failed to clear tp irq\n");
        goto err_enable_irq;
    }
	if (!exp_data.initialized) {
		mutex_init(&exp_data.mutex);
...
static int synaptics_rmi4_resume(struct device *dev)
{
    ...
    if (rmi4_data->staying_awake)
        return 0;
    mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
    synaptics_rmi4_hardware_reset_tp();
    synaptics_rmi4_config_reg_custom(rmi4_data);
	...
	if (rmi4_data->enable_wakeup_gesture) {
        ...
        if(retval < 0)
        {
            printk("synap resume failed \n");
        }
        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
        synaptics_rmi4_clear_irq(rmi4_data);
		goto exit;
        }
    if (rmi4_data->sensor_sleep == true) {
        synaptics_rmi4_sensor_wake(rmi4_data);
...
int synaptics_rmi4_clear_irq(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    if (retval < 0)
    {
        printk("synap clear irq failed\n");
    }
    return retval;
}

/*start to modify for reset TP when resume by jingyuantao@yulong.com in 20150928*/
static void synaptics_rmi4_hardware_reset_tp(void)
{
...
/*    hwPowerOn(TPD_POWER_SOURCE_2800, VOL_2800, "TP");*/
/*    msleep(5);*/
/*    hwPowerOn(TPD_POWER_SOURCE_1800, VOL_2800, "TP");*/
/*    msleep(100);*/
    mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
    msleep(DELAY_HARDWARE_RESET);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);
    msleep(DELAY_HARDWARE_RESET_READY);
    return;
}
/*end to modify for reset TP when resume by jingyuantao@yulong.com in 20150928*/
...
static  int synaptics_rmi4_config_reg_custom(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    else if(glove_input==MODE_WINDOW)
    {
        if(smarthull_switch)
        {
             if(hall_state)
