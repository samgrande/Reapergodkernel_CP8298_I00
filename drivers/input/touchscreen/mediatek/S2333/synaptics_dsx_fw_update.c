...
#include "8690_samsung.h"
#include "SynaImage_shenyue.h"
#include "SynaImage_yeji.h"
#include <cust_eint.h>
#include <mach/mt_boot.h>//open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805
extern void mt_eint_unmask(unsigned int line);
extern void mt_eint_mask(unsigned int line);

extern int synaptics_rmi4_clear_irq(struct synaptics_rmi4_data *rmi4_data);

#define FW_IMAGE_NAME "synaptics/startup_fw_update.img"
#define DO_STARTUP_FW_UPDATE  //open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805
...
u8 product_id_info[11];
u8  *get_RMI_product_id(void)
{
    int i = 0;
    memset(product_id_info,0,sizeof(product_id_info));
    fwu->fn_ptr->read(fwu->rmi4_data,fwu->rmi4_data->f01_query_base_addr+11,&product_id_info[0], 10);
    product_id_info[10]=0;
    printk("SynaF01QueryBase+11=%x\n",fwu->rmi4_data->f01_query_base_addr+11);
    printk("get_RMI_product_id firmware_id=%s\n",product_id_info);

    for( i = 0;i < 11 ; i ++)
        printk("get_RMI_product_id product_id_info[%d]=%x \n",i,product_id_info[i]);
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
    ...
    printk("image_config_id=%x, device_config_id=%x\n",image_config_id,device_config_id); /*open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
...
/*start to open TW auto update fw when normal boot by qinxinjun@yulong.com in 20150805*/
/*begin:  modified by fanli@yulong.com to  upgrade TW config file in 2014.10.08*/
#if 0
int synaptics_fw_updater(unsigned char *fw_data)
{
	int retval;
       printk("Enter synaptics_fw_updater\n");  //added by fanli@yulong.com to  upgrade TW config file in 2014.10.08
	...
	if (!fwu->initialized)
       ...
       printk("synaptics_fw_updater begin upgrade!\n");  //added by fanli@yulong.com to  upgrade TW config file in 2014.10.08
	fwu->ext_data_source = synaImage_Samsung; //modified by fanli@yulong.com to  upgrade TW config file in 2014.10.08
	fwu->config_area = UI_CONFIG_AREA;

    retval = fwu_start_reflash();

    synaptics_rmi4_clear_irq(fwu->rmi4_data);

      printk("synaptics_fw_updater  upgrade over!\n");  //added by fanli@yulong.com to  upgrade TW config file in 2014.10.08
	
	return retval;
}
#endif
#if 1
int synaptics_fw_updater(int Product_ID)//add by liyongfeng
{
    int retval;
    printk("enter synaptics_fw_updater\n");
    if (!fwu)
    ...
    if(Product_ID==1)
    {
     printk("SynaImage_yeji Product_ID==1\n");
     fwu->ext_data_source = SynaImage_yeji;
     fwu->config_area = UI_CONFIG_AREA;
     retval = fwu_start_reflash();
    }
    if(Product_ID==2)
    {
     printk("SynaImage_shenyue Product_ID==2\n");
     fwu->ext_data_source = SynaImage_shenyue;
     fwu->config_area = UI_CONFIG_AREA;
     retval = fwu_start_reflash();
    }
    return retval;
...
#endif
/*end: modified by fanli@yulong.com to  upgrade TW config file in 2014.10.08*/
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    get_RMI_product_id();
    printk("enter fwu_startup_fw_update_work\n");
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    //retval = synaptics_fw_updater(1);
    get_RMI_product_id();
    if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji 0xa1
        retval = synaptics_fw_updater(1);
    else if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue 0xaf
        retval = synaptics_fw_updater(2);
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    if (retval < 0) {
        dev_err(&rmi4_data->i2c_client->dev,
                "%s: Failed to do reflash\n",
...
/*start to modify TW version info to uppercase by qinxinjun@yulong.com in 20150922*/
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
...
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
/*end to modify TW version info to uppercase by qinxinjun@yulong.com in 20150922*/

extern int get_device_info(char *buf);

int synaptics_tp_get_dev_info()
{
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
        get_RMI_product_id();
        fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,sizeof(config_id));
        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
        //sprintf(buf, "TW: %s:syna3508:0x%02x:0x%02x\n","SDC",config_id[1], config_id[3]);
        if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji 0xa1
            sprintf(buf, "TW: %s:syna2333:0x%02x:0x%02x\n",yl_firmware[0], config_id[1], config_id[3]);
        else if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue 0xaf
            sprintf(buf, "TW: %s:syna2333:0x%02x:0x%02x\n",yl_firmware[1], config_id[1], config_id[3]);
        else
            sprintf(buf, "TW: %s:syna2333:0x%02x:0x%02x\n",yl_firmware[2], config_id[1], config_id[3]);
 /*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/       
		get_device_info(buf);
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
        ...
        fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,sizeof(config_id));
        mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
        printk("Config ID: %x %x %x %x ", config_id[0],config_id[1],config_id[2],config_id[3]);

/*start to modify TW version info by qinxinjun@yulong.com in 20150922*/
        if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji 0xa1
            return sprintf(buf, "%s:syna2333:0x%02x:0x%02x\n",yl_firmware[0], config_id[1], config_id[3]);
        else if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue 0xaf
            return sprintf(buf, "%s:syna2333:0x%02x:0x%02x\n",yl_firmware[1], config_id[1], config_id[3]);
        else
            return sprintf(buf, "%s:syna2333:0x%02x:0x%02x\n",yl_firmware[2], config_id[1], config_id[3]);
/*end to modify TW version info by qinxinjun@yulong.com in 20150922*/
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
}

int synaptics_fw_needupdater(void)
{
        unsigned char config_id[4]={0};

        mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
        printk("inter synaptics_needfw_updater\n");
        if (!fwu)
        {
           return -ENODEV;
        }
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
       get_RMI_product_id();
       fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,4);
       printk("SynaImage_yeji[0xb100+3]=%x,SynaImage_shenyue[0xb100+3]=%x,config_id[1]=%x,config_id[3]=%x\n",SynaImage_yeji[0xb100+3],SynaImage_shenyue[0xb100+3],config_id[1],config_id[3]);
       
        if(product_id_info[8]==0x61&&product_id_info[9]==0x31)//yeji:0xa1
        {
             if(SynaImage_yeji[0xb100+3]>config_id[3])
                 return 1;
             else return 0;
             
        }
        if(product_id_info[8]==0x61&&product_id_info[9]==0x66)//shenyue:0xaf
        {
             if(SynaImage_shenyue[0xb100+3]>config_id[3])
                 return 1;
             else return 0;
        }
/*end to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    int boot_mode = 0; /*open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
...
/*start to open TW auto update fw when normal boot by jingyuantao@yulong.com in 20150805*/
    ...
    printk("NORMAL_BOOT = %d : boot_mode = %d\n",NORMAL_BOOT,boot_mode);
