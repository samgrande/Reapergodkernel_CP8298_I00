...
static struct work_struct workTimeOutPwm;
...
int FL_Enable(void)
{
	PK_DBG(" FL_Enable line,g_duty=%d \n",g_duty);
    if(1>g_duty)
    {
    	...
    	if(mt_set_gpio_out(FLASH_GPIO_ENT,GPIO_OUT_ONE)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
    ...
    else
    {
	    ...
	    if(mt_set_gpio_mode(FLASH_GPIO_ENF,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
    	if(mt_set_gpio_dir(FLASH_GPIO_ENF,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
    	if(mt_set_gpio_out(FLASH_GPIO_ENF,GPIO_OUT_ONE)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
        if(mt_set_gpio_mode(FLASH_GPIO_ENT,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
        if(mt_set_gpio_dir(FLASH_GPIO_ENT,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
        if(mt_set_gpio_out(FLASH_GPIO_ENT,GPIO_OUT_ONE)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
...
int FL_Disable(void)
{
	...
	PK_DBG(" FL_Disable FLASH_GPIO_ENF line=%d\n",__LINE__);
    if(mt_set_gpio_mode(FLASH_GPIO_ENF,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
    if(mt_set_gpio_dir(FLASH_GPIO_ENF,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
	if(mt_set_gpio_out(FLASH_GPIO_ENF,GPIO_OUT_ZERO)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
...
int FL_Init(void)
{
    PK_DBG("FL_Init Start\n");
    PK_DBG(" FL_Init FLASH_GPIO_ENT\n");
	if(mt_set_gpio_mode(FLASH_GPIO_ENT,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
    if(mt_set_gpio_dir(FLASH_GPIO_ENT,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
    if(mt_set_gpio_out(FLASH_GPIO_ENT,GPIO_OUT_ZERO)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
    PK_DBG(" FL_Init FLASH_GPIO_ENF\n");
   	if(mt_set_gpio_mode(FLASH_GPIO_ENF,GPIO_MODE_00)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
    if(mt_set_gpio_dir(FLASH_GPIO_ENF,GPIO_DIR_OUT)){PK_DBG("[constant_flashlight] set gpio dir failed!! \n");}
    if(mt_set_gpio_out(FLASH_GPIO_ENF,GPIO_OUT_ZERO)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
    Pwm_hight = 0;
...
//add by liujianfeng3@yulong.com 201500605 start
   ...
   if(Pwm_hight)
    {
         ...
         hrtimer_forward_now(&g_timeOutTimer_Pwm,
                                          ktime_set(0, (g_timeOutTimeUs+60 - g_duty * 10) * 1000));
    }else
    {
          Pwm_hight = 1;
          if(mt_set_gpio_out(FLASH_GPIO_ENT,GPIO_OUT_ONE)){PK_DBG("[constant_flashlight] set gpio failed!! \n");}
...
//add by liujianfeng3@yulong.com 201500605 end
...
static int constant_flashlight_open(void *pArg)
{
	...
	if (0 == strobe_Res)
	{
	    FL_Init();
		timerInit();
        timerInit_Pwm(); //add by liujianfeng3@yulong.com 201500605
	}
	PK_DBG("constant_flashlight_open line=%d\n", __LINE__);
