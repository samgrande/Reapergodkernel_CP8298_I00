...
void getHwVersion()
{
    //start modify for compatible hardware version by miaolei@yulong.com 2015.06.26
    if((hw_version = mt_get_gpio_in(GPIO82)) < 0 ) {
        PK_DBG("[hardware version] get gpio failed!\n");
    }
    //end modify for compatible hardware version by miaolei@yulong.com 2015.06.26
    PK_DBG("[getHwVersion] hw_version = %d\n", hw_version);
}
...
u32 pinSet[3][8] = {
                        //for main sensor
                     {  GPIO_CAMERA_INVALID,//CAMERA_CMRST_PIN, // The reset pin of main sensor uses GPIO10 of mt6306, please call mt6306 API to set
                        GPIO_CAMERA_INVALID,//CAMERA_CMRST_PIN_M_GPIO,   /* mode */
                     ...
                     //for sub sensor
                     {  GPIO_CAMERA_INVALID,//CAMERA_CMRST1_PIN,
                        GPIO_CAMERA_INVALID,//CAMERA_CMRST1_PIN_M_GPIO,
                        GPIO_OUT_ONE,
                        GPIO_OUT_ZERO,
                        CAMERA_CMPDN1_PIN,
    ...
    if (On) {
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI841_MIPI_RAW, currSensorName)))
        {
           PK_DBG("[CAMERA SENSOR] HI841 POWER ON \n");
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI841KERR_MIPI_RAW, currSensorName)))
        {
           PK_DBG("[CAMERA SENSOR] HI841 KERR POWER ON \n");
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
            ...
            (currSensorName && (0 == strcmp(currSensorName,"imx220mipiraw"))))
        {
            ...
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                //start set the mode and direction of pwd pin by miaolei@yulong.com 2015.04.21
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
                //end set the mode and direction of pwd pin by miaolei@yulong.com 2015.04.21
                if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set gpio failed!! (CMPDN)\n");}
            }

        }
        //start modify for add ov5648 of sunny by miaolei@yulong.com 2015.06.09
        else if ((currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5648_MIPI_RAW, currSensorName))) ||
                (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5648_SUNNY_MIPI_RAW, currSensorName))))
        //end modify for add ov5648 of sunny by miaolei@yulong.com 2015.06.09
        {
    	...
    	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_AR1335_MIPI_RAW, currSensorName)))
        {
	    ...
	    */
        ...
        {
             PK_DBG("[CAMERA SENSOR] SP2508 POWER ON \n");
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI841_MIPI_RAW, currSensorName)))
        {
           PK_DBG("[CAMERA SENSOR] HI841 POWER OFF \n");
        ...
        {
           PK_DBG("[CAMERA SENSOR] HI841 KERR POWER OFF \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
        ...
        {
          PK_DBG("[CAMERA SENSOR] SP2508 POWER OFF \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
        ...
        {
           PK_DBG("[CAMERA SENSOR] AR1335 POWER OFF \n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
