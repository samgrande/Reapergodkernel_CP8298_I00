...
static int yulong_accel_Calibration(struct i2c_client *client, char *buf, int bufsize,int count,unsigned long cal_arg)
{
    ...
    msleep(50);
    if(ret = KXTJ2_1009_SetPowerMode(obj->client, TRUE))
    {
    ...
    for (i = 0; i < count; i++)
    {
    if(ret = KXTJ2_1009_ReadSensorData(client, strbuf, KXTJ2_1009_BUFSIZE));
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = kxtj2_get_cust_acc_hw();   //wangyufei@yulong.com modify for gsensor compat at 20141031
...
/* modify by baibo@yulong.com 201150916 begin for debug*/
    #if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
        GSE_LOG("kxtj2: hwmsen_gsensor_add\n");
        hwmsen_gsensor_add(&kxtj2_1009_init_info);
    #else
    GSE_LOG("kxtj2: platform_driver_register\n");
