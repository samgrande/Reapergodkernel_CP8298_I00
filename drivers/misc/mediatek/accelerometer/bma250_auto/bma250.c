 ...
 *wangyufei@yulong.com add gsensor calibration at 20150416
 */
...
static int yulong_accel_Calibration(struct i2c_client *client, char *buf, int bufsize,int count,unsigned long cal_arg)
{
    ...
    msleep(50);
    if(ret = BMA250_SetPowerMode(obj->client, TRUE))
    {
    ...
    for (i = 0; i < count; i++)
    {
    if(ret = BMA250_ReadSensorData(client, strbuf, BMA250_BUFSIZE));
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* add by mayulong 20130930 begin */
//	obj->hw->direction = 4;
  printk("BMA250E direction is %d\n", obj->hw->direction);
  /* add by mayulong 20130930 end */
...
/* add by mayulong 20130925 end */
  ...
  //end by wangyufei@yulong.com for device list at 20140606
	...
	if(err = hwmsen_attach(ID_GRAVITY, &sobj))
	{
		GSE_ERR("attach fail = %d\n", err);
		goto exit_kfree;
	}
...
/* modify by baibo@yulong.com 201150916 begin for debug*/
    #if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
        hwmsen_gsensor_add(&bma250_init_info);
        GSE_LOG("bma250: hwmsen_gsensor_add\n");
    #else
        GSE_LOG("bma250: platform_driver_register\n");
    ...
    #endif
/* modify by baibo@yulong.com 201150916 end for debug*/
	return 0;    
}
/*----------------------------------------------------------------------------*/
