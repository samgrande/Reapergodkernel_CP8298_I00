...
static int n2dm_SetWorkMode(struct i2c_client *client, u8 work_mode)
{
	...
	if(res = n2dm_Update_Sensitivity(client))
	{
...
static int n2dm_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(res = N2DM_ReadAccRawData(client, obj->data))
	{        
		GSE_ERR("I2C error: ret value=%d", res);
		return -4;
	}
...
// end baibo@yulong for acc_calibration 20150906
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			N2DM_ReadAccRawData(client, strbuf);
			if(copy_to_user(data, strbuf, strlen(strbuf)+1))
			{
				err = -EFAULT;
				break;	  
...
#elif defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
	hwmsen_gsensor_add(&n2dm_init_info);
#else
	if(platform_driver_register(&n2dm_driver))
	{
