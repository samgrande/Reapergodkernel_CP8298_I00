...
static char factory_module_id[50] = {0};     //modify for camera device information show in factory mode by miaolei@yulong.com 2015.08.13
...
static imgsensor_struct imgsensor = {
    .mirror = IMAGE_NORMAL,             //mirrorflip information  //modify orientation of display by miaolei@yulong.com 2015.07.30
...
{{ 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Preview 2112*1558
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // capture 4206*3128
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // video
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1280, 720}, //hight speed video
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200},// slim video
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Custom1 (defaultuse preview)
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Custom2
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Custom3
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Custom4
 { 1616, 1216, 1000, 1000, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0000, 0000, 1600, 1200}, // Custom5
 };// slim video
...
static kal_uint16 set_gain(kal_uint16 gain)
{
  ...
  //return gain;
    ...
    LOG_INF("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
        ...
        write_cmos_sensor(0xfd, 0x01);
		//write_cmos_sensor(0x24, reg_gain >> 8);
...
write_cmos_sensor(0x04,0x09);
		...
		write_cmos_sensor(0x06,0x20); //vblank(dummy line?)

write_cmos_sensor(0x24,0xf0); //pga gain 6x
		...
		write_cmos_sensor(0x79,0x42); //p39 p40 cycle for pixel timing
		write_cmos_sensor(0x85,0x0f); //p51 cycle for pixel timing
		write_cmos_sensor(0x09,0x01); //hblank (dummy pixel?) high 4 bits
...
write_cmos_sensor(0x2a,0xea); //bypass dac res, adc range 0.745, vreg counter 0.9
		...
		write_cmos_sensor(0xd0,0x01); //boost2 enable
		write_cmos_sensor(0xd1,0x01);//boost2 start point  3msb
write_cmos_sensor(0xd2,0xd0);
		write_cmos_sensor(0x55,0x10);//cycle for pixel timing
		...
		write_cmos_sensor(0xfb,0x25);//disable BLC
write_cmos_sensor(0xf0,0x00);//offset  0x10
write_cmos_sensor(0xf1,0x00);//0x10
write_cmos_sensor(0xf2,0x00);//0x10
write_cmos_sensor(0xf3,0x00);//0x10

		//write_cmos_sensor(0xfd,0x02); //raw data digital gain
		//write_cmos_sensor(0x00,0x84);
		//write_cmos_sensor(0x01,0x84);
		//write_cmos_sensor(0x03,0x84);
		//write_cmos_sensor(0x04,0x84);

		write_cmos_sensor(0xfd,0x02);
		write_cmos_sensor(0x39,0x04);//3msb
		write_cmos_sensor(0x3a,0xb0);//vertical size 8lsb
		write_cmos_sensor(0x3d,0x03);
		write_cmos_sensor(0x3e,0x20);//horizontal size(Image half horizontal size)


		write_cmos_sensor(0xfd,0x01);//mipi
		write_cmos_sensor(0xb3,0x00);//Mipi Ido powerdown
		write_cmos_sensor(0x93,0x01);//mipi_pll_bias
		write_cmos_sensor(0x9d,0x17);//blc[4:0]
		write_cmos_sensor(0xc5,0x01);//mipi pll enable
		write_cmos_sensor(0xc6,0x00);//mipi pll select
		write_cmos_sensor(0xb1,0x01);//mipi phy shutdown

		write_cmos_sensor(0x8e,0x06);//mipi horizontal size 4msb
		write_cmos_sensor(0x8f,0x40);//mipi horizontal size(column number) 8lsb

		write_cmos_sensor(0x90,0x04);//mipi vertical size 3msb
		write_cmos_sensor(0x91,0xb0);//mipi vertical size(line number) 8lsb

		write_cmos_sensor(0x92,0x01);//mipi data formate(01 RAW10.10 RAW8)
		write_cmos_sensor(0xa1,0x05);//MIPI TRANS SPEED SELECT
		write_cmos_sensor(0xaa,0x01);//hs mode,00 continue,01clk hs
		write_cmos_sensor(0xac,0x01);// mipi enable
        LOG_INF("Exit preview_setting.\n");

#if 0
	#if SP2508_OTP_Enable
	otp_update();
	#endif
...

{
	 ...
	 write_cmos_sensor(0x1c,0x03);//pull down parallel pad
...
write_cmos_sensor(0x0a,0x40);
  ...
  write_cmos_sensor(0x2a,0xea);//bypass dac res, adc range 0.745, vreg counter 0.9
  write_cmos_sensor(0x2c,0xf0);//high 8bit, pldo 2.7v
  write_cmos_sensor(0x8a,0x55);//pixel bias 2uA
  write_cmos_sensor(0x8b,0x55);
  write_cmos_sensor(0x19,0xf3);//icom1 1.7u, icom2 0.6u
  write_cmos_sensor(0x11,0x30);//rst num
...
write_cmos_sensor(0xd2,0xd0);
		write_cmos_sensor(0x55,0x10);//cycle for pixel timing
write_cmos_sensor(0x58,0x28);//30
		write_cmos_sensor(0x5d,0x15);
		write_cmos_sensor(0x5e,0x05);
		write_cmos_sensor(0x64,0x40);
		write_cmos_sensor(0x65,0x00);
...
write_cmos_sensor(0x03,0x03); //exp time, 3 base
write_cmos_sensor(0x04,0x09);
write_cmos_sensor(0x06,0x20); //vblank
write_cmos_sensor(0x24,0xf0); //pga gain 6x
write_cmos_sensor(0x01,0x01); //enable reg write
write_cmos_sensor(0x2b,0xc4); //readout vref
write_cmos_sensor(0x2e,0x20); //dclk delay
...
write_cmos_sensor(0xaa,0x01);
write_cmos_sensor(0xac,0x01);
  LOG_INF("Exit hs_video setting\n");
}

static void slim_video_setting()  // VideoHDSetting
{
	LOG_INF("Enter slim_setting.\n");
write_cmos_sensor(0xfd,0x00);
write_cmos_sensor(0x1c,0x03);
write_cmos_sensor(0x35,0x20); //pll bias
write_cmos_sensor(0x2f,0x08); //pll clk 60M
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if (enable) {
        ...
        write_cmos_sensor(0xfd,0x01);
        //start modify camera show when test camera in fastmmi mode by miaolei@yulong.com 2015.08.14
        write_cmos_sensor(0xb6,0x01);
        write_cmos_sensor(0x01,0x01);
        //end modify camera show when test camera in fastmmi mode by miaolei@yulong.com 2015.08.14
    } else {
      write_cmos_sensor(0xfd,0x01);
        //start modify camera show when test camera in fastmmi mode by miaolei@yulong.com 2015.08.14
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    s_DEVINFO_ccm->device_module = "PC0FL0001A";//can change if got module id
    s_DEVINFO_ccm->device_vendor = "Sanglaishi";
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            LOG_INF("Read sensor id fail,imgsensor_info.sensor_id=0x%x,  sensor_id: 0x%x\n", imgsensor_info.sensor_id, *sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
    }
    sprintf(factory_module_id,"sub_camera:2M-Camera sp2508-kerr\n");
    if (*sensor_id != imgsensor_info.sensor_id) {
