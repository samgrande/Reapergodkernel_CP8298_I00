...
#define OV5648_OFILM_MODULE_ID 0x07
#define OV5648_SUNNY_MODULE_ID 0x01

static DEFINE_SPINLOCK(imgsensor_drv_lock);

extern int get_device_info(char* buf);
static int info_limit = 0;
static char factory_module_id[50] = {0};        //modify for set camera device information in factory mode by miaolei@yulong.com 2015.05.26

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV5648_SENSOR_ID,
...
/* add begin by songte in 2013.6.26 */

#define RG_RATIO_TYPICAL_5648AF 0x0199;
#define BG_RATIO_TYPICAL_5648AF 0x0149;
#define RG_RATIO_TYPICAL_OYL5F03_5648FF 0x0199;
...
#define BG_RATIO_TYPICAL_SUNNY_P5V30A  0x016F; //add by songte in 2013.6.27//0x015A
#define RG_RATIO_TYPICAL_SUNNY_D5V04A  0x0190; //add by weiwenping@yulong.com at 20150630
#define BG_RATIO_TYPICAL_SUNNY_D5V04A  0x017D; //add by weiwenping@yulong.com at 20150630

static unsigned int rg_ratio_typical = 0x0199;	//1C0
static unsigned int bg_ratio_typical = 0x0149;	//1C0
...
static int ov5648_check_otp(unsigned short index)
{
	unsigned char temp;
	unsigned short i;
	...
	if(index<2) {
		...
		write_cmos_sensor(0x3d85, 0x00); // OTP strat address, bank 0
		write_cmos_sensor(0x3d86, 0x0f); // OTP end address
		write_cmos_sensor(0x3d81, 0x01);
		mdelay(10);
		address = 0x3d05 + index*9;
		temp = read_cmos_sensor(address);
		// disable otp read
		...
		// read otp into buffer
		write_cmos_sensor(0x3d84, 0xc0);
		write_cmos_sensor(0x3d85, 0x10); // OTP strat address, bank 1
		write_cmos_sensor(0x3d86, 0x1f); // OTP end address
		write_cmos_sensor(0x3d81, 0x01);
		mdelay(10);
...
/*
	...
	return: 0,
*/
static int ov5648_read_otp(unsigned short index, struct otp_struct* otp)
{
	unsigned char temp;
	unsigned short address;
	unsigned short i;
	...
	uint8_t ov5648_akerr_rg = 0x4C;
	uint8_t ov5648_akerr_bg = 0x50;
	uint8_t ov5648_qtech_rg = 0x58;
	uint8_t ov5648_qtech_bg = 0x61;
	// add by zdq 2012.9.13 end
	printk("!!!!!!!!!!!liuweijie ov5648_read_otp %d\n",index);
	if(index==0) {
		// read otp into buffer
		write_cmos_sensor(0x3d84, 0xc0);
		write_cmos_sensor(0x3d85, 0x00); // OTP strat address, bank 0
		...
		address = 0x3d05 + index*9;
		temp = read_cmos_sensor(address);
		otp->customer_id = (temp & 0x7f);
		otp->module_integrator_id = temp;
		temp = read_cmos_sensor(address + 1);
		otp->lens_id = temp;
		temp = read_cmos_sensor(address + 2);
		otp->rg_ratio = temp;
		printk("!!!!!!!!!!!liuweijie otp->rg_ratio[9~2] %d\n",temp);
		temp = read_cmos_sensor(address + 3);
		otp->bg_ratio = temp;
		printk("!!!!!!!!!!!liuweijie otp->bg_ratio[9~2] %d\n",temp);
		temp = read_cmos_sensor(address + 4);
		otp->user_data[0] = temp;
		temp = read_cmos_sensor(address + 5);
		otp->user_data[1] = temp;
		temp = read_cmos_sensor(address + 6);
		otp->user_data[2] = temp;
		temp = read_cmos_sensor(address + 7);
		otp->light_rg = temp;
		temp = read_cmos_sensor(address + 8);
		otp->light_bg = temp;

		otp->rg_ratio = (otp->rg_ratio<<2) + (otp->user_data[2]>>6);
		otp->bg_ratio = (otp->bg_ratio<<2) + ((otp->user_data[2]>>4) & 0x03);
		otp->light_rg = (otp->light_rg<<2) + ((otp->user_data[2]>>2) & 0x03);
		otp->light_bg = (otp->light_bg<<2) + (otp->user_data[2] & 0x03);
		// disable otp read
		write_cmos_sensor(0x3d81, 0x00);
	...
	// add by zdq 2012.9.10 start
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.module_integrator_id=0x%02x. \n", otp->module_integrator_id);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.lens_id=0x%02x. \n", otp->lens_id);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.rg_ratio=0x%04x. \n", otp->rg_ratio);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.bg_ratio=0x%04x. \n", otp->bg_ratio);
	module_id = otp->module_integrator_id;//add by songte in 2013.6.27
 if (0x03 == otp->customer_id /*&& 0x04 == otp->lens_id*/)
	{
		printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ a-kerr YL885F. \n");
#ifdef RG_RATIO_TYPICAL_YL885F
		rg_ratio_typical = RG_RATIO_TYPICAL_YL885F;
		bg_ratio_typical = BG_RATIO_TYPICAL_YL885F;
#else
		// add by zdq 2012.9.13 begin
		rg_ratio_typical = RG_RATIO_TYPICAL_5648AF;
		bg_ratio_typical = BG_RATIO_TYPICAL_5648AF;
		otp->rg_ratio = (otp->rg_ratio * ov5648_qtech_rg) / ov5648_akerr_rg;
		otp->bg_ratio = (otp->bg_ratio * ov5648_qtech_bg) / ov5648_akerr_bg;
		// add by zdq 2012.9.13 end
#endif
	}
	else if (0x07 == otp->customer_id /*&& 0x04 == otp->lens_id*/)
	{
		printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ OYL 5648FF. \n");
		rg_ratio_typical = RG_RATIO_TYPICAL_OYL5F03_5648FF;
		bg_ratio_typical = BG_RATIO_TYPICAL_OYL5F03_5648FF;
	}else if (0x06 == otp->customer_id)
	{
		printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ qtech 5648FF. \n");
		rg_ratio_typical = RG_RATIO_TYPICAL_F5648DM_5648FF;
		bg_ratio_typical = BG_RATIO_TYPICAL_F5648DM_5648FF;
	}
	...
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.rg_ratio=0x%04x. \n", otp->rg_ratio);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ otp.bg_ratio=0x%04x. \n", otp->bg_ratio);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ rg_ratio_typical=0x%04x. \n", rg_ratio_typical);
	printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ bg_ratio_typical=0x%04x. \n", bg_ratio_typical);
	// add by zdq 2012.9.10 end

	return   module_id; //modify by songte in 2013.6.27    //0;
}
/*
	...
	return 0;
*/
static int ov5648_update_awb_gain(unsigned int R_gain, unsigned int G_gain, unsigned int B_gain)
{
	printk("R_gain:0x%04x, G_gain:0x%04x, B_gain:0x%04x \n", R_gain, G_gain, B_gain);
	if (R_gain > 0x400) {
		write_cmos_sensor(0x5186, (unsigned char)(R_gain >> 8));
		write_cmos_sensor(0x5187, (unsigned char)(R_gain & 0x00ff));
	}

	if (G_gain > 0x400) {
		write_cmos_sensor(0x5188, (unsigned char)(G_gain >> 8));
		write_cmos_sensor(0x5189, (unsigned char)(G_gain & 0x00ff));
	}

	if (B_gain > 0x400) {
		write_cmos_sensor(0x518a, (unsigned char)(B_gain >> 8));
		write_cmos_sensor(0x518b, (unsigned char)(B_gain & 0x00ff));
	}

#ifdef CAMERA_DEBUG
	unsigned char temp = 0;
	unsigned int rGain = 0, gGain = 0, bGain = 0;
	temp = read_cmos_sensor(0x5186);
	rGain = temp<<8;
	temp = read_cmos_sensor(0x5187);
	rGain += temp;
	temp = read_cmos_sensor(0x5188);
	gGain = temp<<8;
	temp = read_cmos_sensor(0x5189);
	gGain += temp;
	temp = read_cmos_sensor(0x518a);
	bGain = temp<<8;
	temp = read_cmos_sensor(0x518b);
	bGain += temp;
	printk("R_gain:0x%04x, G_gain:0x%04x, B_gain:0x%04x \n", rGain, gGain, bGain);
#endif
...
/*
	...
	1, no OTP
*/
static int ov5648_update_otp(void)
{
	...
	if (i == 3) {
		...
		printk("!!!!!!!!!!!%s -- %s -- %d [no valid OTP]\n", __FILE__, __FUNCTION__, __LINE__);
		return 1;
	}

	ret = ov5648_read_otp(otp_index, &current_otp);
	if (ret < 0) {
...
printk("G_gain= %d B_gain %d R_gain %d\n",G_gain,B_gain,R_gain);
		} else {
			// current_otp.bg_ratio < bg_ratio_typical &&
			// current_otp.rg_ratio >= rg_ratio_typical
			R_gain = 0x400;
...
static int  check_module_id(void)
{
	 struct otp_struct current_otp;
	 unsigned int i;
	 unsigned int otp_index;
	 ...
	 for (i = 0; i < 3; i++) {
		temp = ov5648_check_otp(i);
		printk("!!!!!!!!!!!liuweijie ov5648_check_otp return value %d\n",temp);
		if (temp == 2) {
			otp_index = i;
			break;
		}
...
static UINT32 ov5648_cheak_sensor_module_id(void)
{
    kal_uint16 module_integrator_id = 0;
    UINT32 compatible_sensorid = 0xFFFFFFFF;//OV5648MIPI_SENSOR_ID;
    LOG_INF(" gaoatao ov5648 ov5648_cheak_sensor_module_id  ENter\n");
    ...
    if (OV5648_OFILM_MODULE_ID == module_integrator_id)
	{
		...
		compatible_sensorid = OV5648_SENSOR_ID ;
	}
	else
	{
		printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ mode unknow \n");
                sprintf(factory_module_id,"sub camera:5M-Camera ov5648-unknow\n"); //set camera device info by miaolei@yulong.com 2015.05.25
		compatible_sensorid = 0xFFFFFFFF ;
	}
	
	return compatible_sensorid ;
}
/* add end by songte in 2013.6.26 */



static kal_uint32 return_sensor_id(void)
{
	LOG_INF("gaoatao ov5648 enter return_sensor_id \n");
...
{
    LOG_INF("gaoatao ov5648 sensor_init enter\n");
    /*****************************************************************************
...
static void preview_setting(void)
{
    /********************************************************
       ...
       *
       ********************************************************/
	  LOG_INF("gaoatao ov5648 enter preview setting\n");
    ...
    LOG_INF("gaoatao ov5648 exit preview setting\n");
}    /*    preview_setting  */


static void capture_setting(kal_uint16 currefps)
{
    LOG_INF("gaoatao enter capture setting currefps:%d\n", currefps);
    ...
    write_cmos_sensor(0x0100, 0x01); //Stream On
    LOG_INF("gaoatao exit capture_setting \n");
}    /*    capture_setting  */

static void normal_video_setting(kal_uint16 currefps)
{
    LOG_INF("gaoatao enter normal_video_setting currefps:%d\n", currefps);
    ...
    write_cmos_sensor(0x0100, 0x01); //Stream On
    LOG_INF("gaoatao exit normal_video_setting \n");
}    /*    preview_setting  */
...
static void video_1080p_setting(void)
{
    ...
    /********************************************************
       ...
       ********************************************************/
		LOG_INF("gaoatao enter video_1080p_setting \n");
    write_cmos_sensor(0x0100, 0x00); //Stream Off
...
static void video_720p_setting(void)
{
    /********************************************************
       ...
       ********************************************************/
LOG_INF("gaoatao enter video_720p_setting \n");
    write_cmos_sensor(0x0100, 0x00); //Stream Off
...
{
	LOG_INF("gaoatao enter hs_video_setting \n");
...
{
    LOG_INF("gaoatao enter slim_video_setting \n");
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
    LOG_INF("gaoatao enter get_imgsensor_id \n");
    ...
    *sensor_id =ov5648_cheak_sensor_module_id();
    ...
    if (*sensor_id != OV5648MIPI_SENSOR_ID)
    {
        ...
        *sensor_id = 0xFFFFFFFF;
        LOG_INF("OV5648GetSensorID 2222  *sensorID = 0x%x ", *sensor_id);
        return ERROR_SENSOR_CONNECT_FAIL;
    }
    ...
    LOG_INF("OV5648GetSensorID 3333 *sensorID = 0x%x ", *sensor_id);
    //modify display camera info in EngMode by weiwenping@yulong.com at 20150521
...
/* add end by gaoatao 20140920*/
    ...
    spin_unlock(&imgsensor_drv_lock);
LOG_INF("gaoatao ov5648 exit , id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
    return ERROR_NONE;
}    /*    open  */
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     LOG_INF("gaoatao normal_video enter\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_INF("gaoatao hs_video enter\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    LOG_INF("gaoatao slim_video enter\n");
...
{
   LOG_INF("gaoatao get_resolution enter\n");
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	  LOG_INF("gaoatao get_info enter\n");
...
{
	  LOG_INF("gaoatao set_auto_flicker_mode enter\n");
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    kal_uint32 frame_length;
		LOG_INF("gaoatao set_max_framerate_by_scenario enter\n");
...
{
	  LOG_INF("gaoatao get_default_framerate_by_scenario enter\n");
    LOG_INF("scenario_id = %d\n", scenario_id);
...
{
    LOG_INF("gaoatao set_test_pattern_mode enter\n");
    LOG_INF("enable: %d\n", enable);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    LOG_INF("gaoatao feature_control enter\n");
...
UINT32 OV5648MIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    ...
    LOG_INF("gaoatao OV5648MIPISensorInit enter\n");
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
