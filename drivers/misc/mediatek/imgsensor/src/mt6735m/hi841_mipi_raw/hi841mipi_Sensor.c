...
static kal_uint16 my_cnt=0;

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI841_SENSOR_ID,
	
	.checksum_value = 0x929ed8f9,            //0x6d01485c
...
{ 3264, 2448,	 0,    0, 3264, 2448,  1632, 1224, 0, 0, 1632, 1224,		0,	  0, 1632, 1224}, // capture
 ...
 { 3264, 2448,     0,  0, 3264, 2448, 3264, 2448, 0, 0, 1280,  720,      0,    0, 1280,  720}};// slim video
...
#define HI841MIPI_USE_OTP   1
extern int get_device_info(char* buf);
static int info_limit = 0;
static  kal_uint16 g_otp_id = 0;
static char factory_module_id[50] = {0};
//opt codes
#define OTP_START_ADDR 0xA000
...
// index: index of otp group. (1, 2,°À¢Á¢Æi, OTP_DRV_INFO_GROUP_COUNT)
...
{
	int flag;
	int nFlagAddress = 0xbd00 + (index -1) * 32 - OTP_START_ADDR;

	write_cmos_sensor(0x8400, 0x01); //write_cmos_sensor
	msleep(1);

	write_cmos_sensor( 0x9c04, ((nFlagAddress)>>8) & 0xff);//72
	write_cmos_sensor( 0x9c05, (nFlagAddress) & 0xff);//20
	msleep(1);

	write_cmos_sensor( 0x9c00, 0x11);
	msleep(1);
	flag = read_cmos_sensor( OTP_START_ADDR);//40
	write_cmos_sensor( 0x9c00, 0x10);
	printk(KERN_ERR "check otp info index %d flag = %x", index, flag);
	return flag;
...
// 1, group index has invalid data
// 2, group index has valid data
static int check_otp_lsc(int index)
{
	int flag;
	uint16_t nFlagAddress = 0xbce2 + (index - 1) * 2 - 0xA000;
	...
	printk(KERN_ERR "check otp lsc index %d flag = %x", index, flag);
	return flag;
}
// index: index of otp group. (1, 2,...,OTP_DRV_INFO_GROUP_COUNT)
// otp_ptr: pointer of otp_struct
// return: 0,
static int read_otp_info_wb(int index, struct otp_struct *otp_ptr)
{
	int start_addr = 0xbd01 + (index - 1) * 32 - OTP_START_ADDR;
	uint16_t r_gain = 0,
		 b_gain = 0;
	...
	write_cmos_sensor( 0x8400, 0x01);
	msleep(1);

	write_cmos_sensor( 0x9c04, ((start_addr) >> 8) & 0xff);//72
	write_cmos_sensor( 0x9c05, (start_addr) & 0xff);//20
	msleep(1);

	write_cmos_sensor( 0x9c00, 0x11);
	msleep(1);

	(*otp_ptr).module_integrator_id = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).lens_id = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).vcm_id = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).driver_id = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).production_year = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).production_month = read_cmos_sensor( OTP_START_ADDR);
	(*otp_ptr).production_day = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_rg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_rg_l = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_bg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_bg_l = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_gg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_unit_gg_l = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_rg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_rg_l = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_bg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_bg_l = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_gg_h = read_cmos_sensor( OTP_START_ADDR);
	wb_golden_gg_l = read_cmos_sensor( OTP_START_ADDR);

	LOG_INF(KERN_ERR"module_integrator_id : %d, (*otp_ptr).lens_id:%d, (*otp_ptr).vcm_id:%d\n", (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id, (*otp_ptr).vcm_id);

	write_cmos_sensor( 0x9c00, 0x10);

	msleep(1);

	rg_golden_value = ((wb_golden_rg_h << 8) | wb_golden_rg_l);
	bg_golden_value = ((wb_golden_bg_h << 8) | wb_golden_bg_l);
	(*otp_ptr).golden_rg_ratio = rg_golden_value;
	(*otp_ptr).golden_bg_ratio = bg_golden_value;

	rg_value = ((wb_unit_rg_h << 8) | wb_unit_rg_l);
	bg_value = ((wb_unit_bg_h << 8) | wb_unit_bg_l);
	LOG_INF("%s,Read from module: rg_golden_typical = 0x%x, bg_golden_typical = 0x%x\n",__func__, rg_golden_value, bg_golden_value);
	LOG_INF("%s,Read from module: rg_value = 0x%x, bg_value = 0x%x\n",__func__, rg_value, bg_value);

	r_gain = (0x100 * rg_golden_value) / rg_value  ;
	b_gain = (0x100 * bg_golden_value) / bg_value ;
	LOG_INF("%s,After calcuate: r_gain = 0x%x, b_gain = 0x%x\n", __func__, r_gain, b_gain);
	LOG_INF("%s exit\n", __func__);
...
// otp_ptr: pointer of otp_struct
static int update_lsc(int index)
{
	uint16_t start_addr = 0xa890 + (index - 1) * 0x6c6 - OTP_START_ADDR;
	uint16_t temp;
	msleep(1);

	write_cmos_sensor( 0x9c04, ((start_addr) >> 8) & 0xff);//72
	write_cmos_sensor( 0x9c05, (start_addr) & 0xff);//20
	write_cmos_sensor( 0x9c06, 0x00);//20
	write_cmos_sensor( 0x9c07, 0x00);//20

	write_cmos_sensor( 0x9c02, 0x06);//20
	write_cmos_sensor( 0x9c03, 0xc5);//20

	write_cmos_sensor( 0x9c08, 0x02); //SRAM Data Width (Almost 0x02 24bit)
	write_cmos_sensor( 0x9c0a, 0x02); //LSC Bank0 Sel.
	write_cmos_sensor( 0x9c00, 0x18); //LSC DMA Enable
	write_cmos_sensor( 0x5c02, 0x01); //LSC bank0

	temp = read_cmos_sensor( 0x5c00) | 0x01;
	write_cmos_sensor( 0x5c00, temp); //LSC enable

	return 0;
}

// call this function after HI841 initialization
// return value: 0 update success
// 1, no OTP
static int update_otp_wb()
{
	uint8_t temp;
	printk("R_gain = 0x%x, B_gain = 0x%x\n", current_otp.r_gain, current_otp.b_gain);
	write_cmos_sensor( 0x0210, current_otp.r_gain >> 8);
	write_cmos_sensor( 0x0211, current_otp.r_gain & 0xff);
	write_cmos_sensor( 0x0212, current_otp.b_gain >> 8);
	write_cmos_sensor( 0x0213, current_otp.b_gain & 0xff);
	temp = read_cmos_sensor( 0x6000) | 0x03;
	write_cmos_sensor( 0x6000, temp);
	return 0;
}
// call this function after HI841 initialization
// return value: 0 update success
// 1, no OTP
static int update_otp_lsc()
{
	int i;
	int otp_index = 0;
	int temp;
	...
	if (i>OTP_DRV_LSC_GROUP_COUNT) {
		...
		current_otp.r_gain = 0;
		current_otp.b_gain = 0;
		return 1;
	}
	printk(KERN_ERR "check otp lsc index %d", otp_index);
	update_lsc( otp_index); // success
	return 0;
}


static UINT32 hi841_cheak_module_id(void)
{
	int i;
	int otp_index;// bank 4,8,12
	int temp;
	UINT32 compatible_sensorid = 0xFFFFFFFF;

	printk(KERN_ERR "gaoatao moudle %s ", __func__);
	// check first lens correction OTP with valid data
	for (i = 1; i <= 3; i++) {
	...
	read_otp_info_wb(otp_index, &current_otp);

	 g_otp_id = current_otp.module_integrator_id;
	printk("gaoatao get g_otp_id = %x\n",g_otp_id);
		if (0x01 == g_otp_id)
		{
			printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ hi841 kerr \n");
			sprintf(factory_module_id,"main_camera:8M-Camera hi841-kerr\n");
			//compatible_sensorid = HI841KERR_SENSOR_ID;
			compatible_sensorid = HI841_SENSOR_ID ;
		}
		else if (0x02 == g_otp_id)
		{
			printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ hi841 zhengqiao. \n");
			sprintf(factory_module_id,"main_camera:8M-Camera hi841-zhengqiao\n");
			compatible_sensorid = HI841_SENSOR_ID ;
		}
		else
		{
			printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ mode unknow \n");
			sprintf(factory_module_id,"main camera: mode unknow \n");
		}

	return compatible_sensorid ;
...
static void set_shutter(kal_uint16 shutter)
{
	...
	//shutter=1024;  //modified by wenwu.xu for debug usage
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if(my_cnt%8==0)
		...
		gain=100;
	LOG_INF("wenwu gain = %d , reg_gain = 0x%x,cnt=%d ", gain, reg_gain,my_cnt);
	#endif
...
write_cmos_sensor(0x41C3, 0x0C);
write_cmos_sensor(0x41C0, 0x03);
write_cmos_sensor(0x41C1, 0xE6);
...
static void preview_setting(void)
{
	...
	//	MCLK/PCLK		: 24MHz / 153.6Mhz 
	//	MIPI speed(Mbps): 760Mbps(each lane), MIPI 4lane
	//	Frame Length	: 1260
	//	Line Length : 4008	
	//	Max Fps 		: 30fps 
	//	Pixel order 	: Green 1st (=GB)
...
write_cmos_sensor(0x5c07, 0xD6);
	...
	// MIPI Setting 
write_cmos_sensor(0x0800, 0x57);                                               
write_cmos_sensor(0x0801, 0x1F);                                              
write_cmos_sensor(0x0802, 0x09);                            
write_cmos_sensor(0x0803, 0x26);                                           
write_cmos_sensor(0x0804, 0x1F);                                 
write_cmos_sensor(0x0805, 0x17);                                            
write_cmos_sensor(0x0806, 0x70);                               
write_cmos_sensor(0x0807, 0x1F);  
...
write_cmos_sensor(0x7407, 0x00);                                                       
	...
	write_cmos_sensor(0x0106, 0x01);//solved preview pruple screen
...
write_cmos_sensor(0x0100, 0x01);  // sleep oFF
...
write_cmos_sensor(0x0802, 0x09);  // ths prepare + ths_zero_min 257ns                             
write_cmos_sensor(0x0803, 0x26);  // ths_trail 78ns                                               
write_cmos_sensor(0x0804, 0x1F);  // tclk_trail_min 70ns                                          
write_cmos_sensor(0x0805, 0x17);  // tclk_prepare 64ns                                            
write_cmos_sensor(0x0806, 0x70);  // tclk prepare + tclk_zero 406ns                               
...
write_cmos_sensor(0x7407, 0x00);  // pll multiplier                                               
...
LOG_INF("[get_imgsensor_id] ");
	...
	LOG_INF("Wenwu.xu read sensor id force to OK");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
				...
				LOG_INF("get_imgsensor_id Read sensor id fail, i2c_write_id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
  ...
  //*sensor_id = imgsensor_info.sensor_id;
	...
	#endif
    *sensor_id=hi841_cheak_module_id();
		printk("hi841GetSensorID *sensorID = 0x%x ", *sensor_id);
    if (*sensor_id != HI841_SENSOR_ID) {
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }else if(info_limit<1){
      printk("sinfo_limit<1\n");
...
{
	LOG_INF("[Wenwu.xu open] ");
#if 1  
	//const kal_uint8 i2c_addr[] = {IMGSENSOR_WRITE_ID_1, IMGSENSOR_WRITE_ID_2};
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	kal_uint16 sensor_id = 0; 
	LOG_INF("[open]: PLATFORM:MT6752,MIPI 4LANE\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("open:Read sensor id fail open w, i2c_write_id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
#ifdef HI841MIPI_USE_OTP
   if(update_otp_lsc()!=0)
		{
					LOG_INF("HI841 LSC in no OTP \n ");
		}
		else
		{
		      LOG_INF("HI841_OTP_LSC is  update successful \n ");
		}
		if(update_otp_wb()!=0)
		{
					LOG_INF("HI841 AWB no  OTP \n ");
		}
		else
		{
		    	LOG_INF("HI841_OTP_AWB  is  update successful \n ");
		}
#endif
...
static kal_uint32 control(MSDK_SCENARIO_ID_ENUM scenario_id, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    bool flag_preview_to_video=FALSE;
	//static int cnt_switch=0;
	LOG_INF("[contrlo]scenario_id = %d,previous scenario_id=%d\n", scenario_id,imgsensor.current_scenario_id);
	spin_lock(&imgsensor_drv_lock);
	
	//if(imgsensor.current_scenario_id == scenario_id){
		
	//flag_preview_to_video=TRUE;
	//++;	 
	//	}
	...
	switch (scenario_id) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		    //if(flag_preview_to_video==TRUE&&cnt_switch!=0){
			//LOG_INF("enter into video or back from video to preview ,not cal preview() again\n");
		    //	}
			//else{
				LOG_INF("preview\n");
...
static kal_uint32 set_auto_flicker_mode(kal_bool enable, UINT16 framerate)
{
	...
	LOG_INF("set_auto_flicker_mode enter \n");
