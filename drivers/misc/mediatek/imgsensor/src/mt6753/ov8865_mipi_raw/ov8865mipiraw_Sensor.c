...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x380e,0x05);// ; VTS H
	write_cmos_sensor(0x380f,0x00);// ; VTS L
...
{
	LOG_INF("Enter! liuweijie test 222 currefps:%d\n",currefps);
	...
	{
		LOG_INF("liuweijie Enter! 444currefps:%d\n",currefps);
		write_cmos_sensor(0x0100,0x00);

		write_cmos_sensor(0x0302,0x1e);// ; PLL
		write_cmos_sensor(0x030d,0x1e);// ; PLL	  ;1e
		...
		write_cmos_sensor(0x382a,0x01);// ; Y inc odd
		write_cmos_sensor(0x382b,0x01);// ; Y inc even
		write_cmos_sensor(0x382d,0x7f);// ; Y inc even
		write_cmos_sensor(0x3830,0x09);// ; ablc_use_num[5:1]
		write_cmos_sensor(0x3836,0x01);// ; zline_use_num[5:1]
		write_cmos_sensor(0x3846,0x48);// ; Y/X boundary pixel numbber for auto size mode
		write_cmos_sensor(0x3f08,0x16);//
		write_cmos_sensor(0x4000,0xf1);// ; our range trig en, format chg en, gan chg en, exp chg en, median en
		write_cmos_sensor(0x4001,0x04);// ; left 32 column, final BLC offset limitation enable
		write_cmos_sensor(0x4006,0x01);// ; revise for ZSD ON/OFF unstable,MTK
		write_cmos_sensor(0x4007,0x01);//
		write_cmos_sensor(0x4020,0x00);// ; anchor left start H
		write_cmos_sensor(0x4021,0x04);// ; anchor left start L
		write_cmos_sensor(0x4022,0x0a);// ; anchor left end H
		write_cmos_sensor(0x4023,0x9b);// ; anchor left end L
		write_cmos_sensor(0x4024,0x0c);// ; anchor right start H
		write_cmos_sensor(0x4025,0x9c);// ; anchor right start L
		write_cmos_sensor(0x4026,0x0c);// ; anchor right end H
		write_cmos_sensor(0x4027,0xcb);// ; anchor right end L
		write_cmos_sensor(0x402a,0x04);// ; top black line start
		write_cmos_sensor(0x402b,0x0c);// ; top black line number
		write_cmos_sensor(0x402c,0x02);// ; bottom zero line start
		write_cmos_sensor(0x402d,0x02);// ; bottom zero line number
		write_cmos_sensor(0x402e,0x10);// ; bottom black line start
		write_cmos_sensor(0x4500,0x68);// ; ADC sync control
		write_cmos_sensor(0x4601,0x10);// ; V FIFO control
		write_cmos_sensor(0x4837,0x16);// ; clock period
		write_cmos_sensor(0x5002,0x08);// ; vario pixel off
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x3846,0x48);// ; Y/X boundary pixel numbber for auto size mode
	write_cmos_sensor(0x3f08,0x16);//
	write_cmos_sensor(0x4000,0xc1);//; our range trig en, format chg en, gan chg en, exp chg en, median en
	write_cmos_sensor(0x4001,0x04);// ; left 32 column, final BLC offset limitation enable
	...
	write_cmos_sensor(0x4027,0xcb);// ; anchor right end L
	write_cmos_sensor(0x402a,0x04);// ; top black line start
	write_cmos_sensor(0x402b,0x0c);//am26
	write_cmos_sensor(0x402c,0x02);// ; bottom zero line start
	write_cmos_sensor(0x402d,0x02);// ; bottom zero line number
	write_cmos_sensor(0x402e,0x10);// ; bottom black line start
	write_cmos_sensor(0x4500,0x68);// ; ADC sync control
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if(*sensor_id == imgsensor_info.sensor_id)
    {
        if(OV8865_OFILM_MODULE_ID == slect_Typical_index())
        {
            sprintf(factory_module_id,"sub_camera:8M-Camera OV8865-Oflim\n");
            *sensor_id = OV8865_SENSOR_ID;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (sensor_id == imgsensor_info.sensor_id) {
                               BG_Ratio_Typical_8865=289;
                                RG_Ratio_Typical_8865=269;
                                   break;
