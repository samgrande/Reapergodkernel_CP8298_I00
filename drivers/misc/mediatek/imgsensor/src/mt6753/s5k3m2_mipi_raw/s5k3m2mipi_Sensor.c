...
extern  kal_uint8 lscotpdata[];
extern int get_device_info(char* buf);//add for factory by wangyi
static int gs_info_limit = 0;
static char factory_module_id[30] = {0};//modify for factory mode cheak id by wangyi
...
static imgsensor_info_struct imgsensor_info = {
	...
	.cap1 = {
		.pclk = 266600000,        // 266600000
		.linelength = 4592,
		.framelength = 3870,      // 3870
		.startx = 0,
		.starty = 0,
	...
	.normal_video = {
		...
		.startx = 0,
		.starty = 0,
        .grabwindow_width = 2096,//4192,//5334,//modified by MTK
        .grabwindow_height = 1552,//3104,//modified by MTK
		.mipi_data_lp2hs_settle_dc = 85,//unit , ns
		.max_framerate = 300,
	},
	.hs_video = {
    ...
    .i2c_speed = 400, // i2c read/write speed //modified by gaoatao@yulong.com for i2c speed 400k 2015-8-06
};
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 4192, 3104,	  0,  0, 4192, 3104, 2096,  1552, 0000, 0000, 2096, 1552, 0,    0, 2096,  1552}, // video
...
//ADD by gaoatao for otp start


extern int S5K3M2_Read_LSC_Otp(u32 a_u2Addr ,u32 Outdatalen,unsigned char * pOutputdata);


kal_uint16 S5K3M2_MIPI_read_EEPROM(kal_uint16 addr,kal_uint8 EEPROMID )
{
    kal_uint16 get_byte=0;
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    char puSendCmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF)};
    iReadRegI2C(puSendCmd , 2, (u8*)&get_byte,1,EEPROMID);
    return get_byte;
}
//ADD by gaoatao for otp end
...
LOG_INF("-- gaoatao, chip id = 0x%x\n",chip_id);
  ...
  write_cmos_sensor(0x6F12, 0x0040);  //ob for full size
  write_cmos_sensor(0x6F12, 0x0040);  //ob for bining
  ...
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x14F0);
  write_cmos_sensor(0x6F12, 0x0040); //OB must=64 when PDAF
  write_cmos_sensor(0x6F12, 0x0040); //OB must=64 when PDAF
  write_cmos_sensor(0x0b00, 0x0000); //dynamic bpc must off when using PDAF

  //For power consumption, need apply straong MTK bpc patch
  write_cmos_sensor_8(0x0B05,0x00);
  write_cmos_sensor_8(0x0B08,0x00);
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x192C);
  write_cmos_sensor(0x6F12, 0x0001);
  	mdelay(2);
  // Stream On
...
//	otp_lsc_update();
  ...
  write_cmos_sensor(0xF49E, 0x004C);
  write_cmos_sensor(0xF4A6, 0x00F0);
//  write_cmos_sensor(0x3AFA, 0xFBB8);   //eliminated
  write_cmos_sensor(0xF49C, 0x0000);
  write_cmos_sensor(0xF496, 0x0000);
  write_cmos_sensor(0xF476, 0x0040);
  write_cmos_sensor_8(0x3AAA, 0x02);
  ...
  write_cmos_sensor(0x3A92,0x0807);
...
write_cmos_sensor(0x32CA,0x022B);
write_cmos_sensor(0x32D6,0x022B);
write_cmos_sensor(0x0344,0x0010);//bpc off, no matter mirrir flip, PV & cap FOV the same
write_cmos_sensor(0x0346,0x0010);//bpc off, no matter mirrir flip, PV & cap FOV the same
write_cmos_sensor(0x0348,0x106F);//bpc off, no matter mirrir flip, PV & cap FOV the same
write_cmos_sensor(0x034A,0x0C2F);//bpc off, no matter mirrir flip, PV & cap FOV the same
...
write_cmos_sensor(0x6F12,0x0040);
write_cmos_sensor(0x6028,0x4000);//#smiaRegs_rw_frame_timing_x_addr_start	 4
write_cmos_sensor(0x0344,0x0010);//#smiaRegs_rw_frame_timing_y_addr_start	 4
write_cmos_sensor(0x0346,0x0010);//#smiaRegs_rw_frame_timing_x_addr_end	 4219
write_cmos_sensor(0x0348,0x106F);//bpc off, no matter mirrir flip, PV & cap FOV the same
write_cmos_sensor(0x034A,0x0C2F);//bpc off, no matter mirrir flip, PV & cap FOV the same
write_cmos_sensor(0x034C,0x0830);
write_cmos_sensor(0x034E,0x0610);
...
write_cmos_sensor(0x034A,0x0C2F);//bpc off, no matter mirrir flip, PV & cap FOV the same
	...
	write_cmos_sensor(0x0308,0x0008);
	write_cmos_sensor(0x0342,0x11F0);
	write_cmos_sensor(0x0340,0x0F1E);
	write_cmos_sensor(0x0202,0x0200);
	write_cmos_sensor(0x0200,0x0400);
	write_cmos_sensor(0x3BBC,0x0000);
	write_cmos_sensor(0x3BBE,0x0000);
...
write_cmos_sensor(0x034A,0x0C2F);//bpc off, no matter mirrir flip, PV & cap FOV the same
	...
	write_cmos_sensor(0x0136,0x1800);
	write_cmos_sensor(0x0304,0x0006);
	write_cmos_sensor(0x0306,0x0064);  // init 64
	write_cmos_sensor(0x0302,0x0001);
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
            if (*sensor_id == imgsensor_info.sensor_id) {
              ...
              //  S5K3M2_Read_LSC_Otp(0x17,1868,lscotpdata);
                S5K3M2_Read_LSC_Otp(0x17,1868,lscotpdata);
                break;
            }
...
//modified start by gaoatao@yulong.com for get module id 2015-5-26 13:47:19
    g_otp_id = S5K3M2_MIPI_read_EEPROM(0x01,0xA0);
    LOG_INF("S5K3M2 g_otp_id 02 = %x\n",g_otp_id);
    switch(g_otp_id)
    {
        case 0x01:
            LOG_INF("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ S5K3M2.sunny \n");
            *sensor_id = S5K3M2_SENSOR_ID ;
            sprintf(factory_module_id,"Camera:13M-Camera S5K3M2-sunny\n");
            break;
        case 0x03:
            LOG_INF("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ S5K3M2.qtech \n");
            *sensor_id = S5K3M2_SENSOR_ID ;
            sprintf(factory_module_id,"Camera:13M-Camera S5K3M2-qtech\n");
            break;
        default:
            LOG_INF("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ mode unknow \n");
            *sensor_id = 0xFFFFFFFF;
    ...
    if(gs_info_limit<1)
    {
        LOG_INF("gs_info_limit<1\n");
        get_device_info(factory_module_id);
        gs_info_limit++;
    }

    LOG_INF(" gaoatao S5K3M2GetSensorID = 0x%04x\n", *sensor_id);
//modified end by gaoatao@yulong.com for get module id 2015-5-26 13:47:19
    return ERROR_NONE;
}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_3m2_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
