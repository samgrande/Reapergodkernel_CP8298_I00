...
/*start qinxinjun@yulong.com 20141024 add smart hull for window mode*/
//#if defined(CP8675)
extern void hall_state_notify_touch(int state);     //add by wangyufei@yulong.com for cover model at 20150519
//#endif 
/*end qinxinjun@yulong.com 20141024 add smart hull for window mode*/
static void hall_poing_callback(struct work_struct *work)
{
		int hall_value;
		
		hall_value= mt_get_gpio_in(GPIO_HALL_1_PIN);
		printk("HALL_callback1111111111wangyufei11111111!!!!\n"); 
...
/*start qinxinjun@yulong.com 20141024 add smart hull for window mode*/
//#if defined(CP8675)
                hall_state_notify_touch(0);     //add by wangyufei@yulong.com for cover model at 20150519
//#endif 
/*end qinxinjun@yulong.com 20141024 add smart hull for window mode*/
				if(atomic_read(&hall_suspend))
				{	   		
						printk("HALL_hall_suspend_wangyufei\n"); 
		...
		{
				printk("HALL_no_suspend_wangyufei\n"); 
				mt_eint_set_polarity(CUST_EINT_HALL_1_NUM, MT_EINT_POL_POS);
				mt_eint_set_sens(CUST_EINT_HALL_1_NUM, MT_LEVEL_SENSITIVE);
				switch_set_state((struct switch_dev *)&hall_data, 1);
/*start qinxinjun@yulong.com 20141024 add smart hull for window mode*/
//#if defined(CP8675)
            hall_state_notify_touch(1);     //add by wangyufei@yulong.com for cover model at 20150519
//#endif 
/*end qinxinjun@yulong.com 20141024 add smart hull for window mode*/
...
static int hall_probe(struct platform_device *pdev) 
{
	...
	atomic_set(&hall_suspend, 0);
        ...
        mt_eint_set_hw_debounce(CUST_EINT_HALL_1_NUM, 256);
	ret = mt_get_gpio_in(GPIO_HALL_1_PIN);
        if(ret)
	{
       		mt_eint_registration(CUST_EINT_HALL_1_NUM, CUST_EINTF_TRIGGER_LOW, detect_hall_handler, 1);
	} 
        else
	{
    		mt_eint_registration(CUST_EINT_HALL_1_NUM, CUST_EINTF_TRIGGER_HIGH, detect_hall_handler, 1);
	}
