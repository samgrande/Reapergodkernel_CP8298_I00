...
#include <linux/ktime.h>//2015.08.07 lijianbin@yulong.com add for MTK patch 80
#include <linux/of.h>
...
extern DSI_manual_enter_HS(cmdqRecHandle cmdq);
extern DSI_sw_clk_trail_cmdq(int module_idx,cmdqRecHandle cmdq);
...
static int is_hwc_update_frame; //modify for Patch ALPS02103525 by xuzhixian@yulong.com 20150604
...
static disp_internal_buffer_info *ext_decouple_buffer_info[DISP_INTERNAL_BUFFER_COUNT];
static int ext_dc_buf_id = 0;
#endif
...
//start 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
extern unsigned int gEnableDSIStateCheck;
//end 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
	...
	if(ctx && head)
	{
		...
		if(ion_client == NULL)
		{
			...
			if(buf)
			{
				...
				ion_phys(ion_client, src_handle, (ion_phys_addr_t*)&src_mva, (size_t*)&tmp_size);
				if(tmp_size == 0)
				{
				...
				buf->mva = src_mva;
				buf->size = tmp_size;
				DISPMSG("buf:0x%p, buf->list:0x%lx, buf->mva:0x%08x, buf->size:0x%08x, buf->handle:0x%p\n",
				  buf, (unsigned long)(&buf->list), buf->mva, buf->size, buf->handle);
			}
			else
			{
...
//modify for P81 by xuzhixian@yulong.com 20150821 start
...
static void primary_suspend_release_present_fence(void)
{
	...
	disp_sync_info *layer_info = _get_sync_info(pgc->session_id, disp_sync_get_present_timeline_id());
	if(layer_info == NULL) {
    ...
    if(primary_display_is_video_mode())
   	{
		...
		if(enter)
		{
			dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_vfp, DDP_DSI_VFP_LP, (unsigned long*) &(lcm_param->dsi.vertical_vfp_lp));
		}else{
			dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_vfp, DDP_DSI_VFP_LP, (unsigned long*) &(lcm_param->dsi.vertical_frontporch));
		}
...
{
	static unsigned int isLowPowerMode = 0;
    ...
    if (primary_get_state() == DISP_SLEEPED)//modify for P81 by xuzhixian@yulong.com 20150821
	{
    ...
    if(disp_low_power_enlarge_blanking==1)
    {
    	...
    	if(enter)
    	{
    		DISP_REG_SET(pgc->cmdq_handle_config, dispsys_reg[DISP_REG_DSI0]+DISP_DSI_REG_VFP, vfp_for_low_power);
    	}
    	else
    	{
    		DISP_REG_SET(pgc->cmdq_handle_config, dispsys_reg[DISP_REG_DSI0]+DISP_DSI_REG_VFP, vfp_original);
    	}
#else
    ...
    if(1==disp_low_power_disable_ddp_clock)  //open
    {
        ...
        {
            static unsigned int disp_low_power_disable_ddp_clock_cnt = 0;
        ...
        if(primary_display_is_video_mode()==1)  // only for video mode
        {
            ...
            if(enter)            {
                 ...
                 if(pgc->session_mode==DISP_SESSION_DIRECT_LINK_MODE)
                 {
                     ...
                     primary_display_switch_mode_nolock(DISP_SESSION_DECOUPLE_MODE, pgc->session_id, 1);
                 }
            ...
            else
            {
                 ...
                 {
                     DISPDBG("[LP]add ovl. \n");
                     primary_display_switch_mode_nolock(DISP_SESSION_DIRECT_LINK_MODE, pgc->session_id, 1);
                 }
...
static int _disp_primary_path_idle_detect_thread(void* data)
{
	...
	while(1)
	{
        ...
        if(gSkipIdleDetect || atomic_read(&isDdp_Idle)==1 || is_hwc_update_frame == 0)  // skip is already in idle mode //modify for Patch ALPS02103525 by xuzhixian@yulong.com 20150604
		{
            continue;
		}
		_primary_path_lock(__func__);
        if(primary_get_state() != DISP_ALIVE)//modify for P81 by xuzhixian@yulong.com 20150821
		{
...
int primary_display_get_debug_state(char* stringbuf, int buf_len)
{
	...
	len += scnprintf(stringbuf+len, buf_len - len, "|cmdq_handle_config=%p\tcmdq_handle_trigger=%p\tdpmgr_handle=%p\tovl2mem_path_handle=%p\n", pgc->cmdq_handle_config, pgc->cmdq_handle_trigger, pgc->dpmgr_handle, pgc->ovl2mem_path_handle);
...
#endif
    hrtimer_forward_now(timer, ns_to_ktime(16666666));//2015.08.07 lijianbin@yulong.com add for MTK patch 80
...
//end 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
	...
	if(primary_display_is_video_mode())
	{
        ...
        ret = cmdqRecWait(pgc->cmdq_handle_trigger, CMDQ_EVENT_DISP_RDMA0_EOF);//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
...
//start 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
        ...
        if(gEnableDSIStateCheck==1)
        {
            for (i = 0; i<10; i++)
            {
                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->dsi_state_info, i, DISP_REG_DSI_STATE);

                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->rdma_state_info, i*5+0, DISP_REG_RDMA_INT_STATUS);
                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->rdma_state_info, i*5+1, DISP_REG_RDMA_IN_P_CNT);
                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->rdma_state_info, i*5+2, DISP_REG_RDMA_IN_LINE_CNT);
                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->rdma_state_info, i*5+3, DISP_REG_RDMA_OUT_P_CNT);
                cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_trigger, pgc->rdma_state_info, i*5+4, DISP_REG_RDMA_OUT_LINE_CNT);
            }
        }
//end 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016

    // SODI is disabled in config thread, so mutex enable/dsi start/CPU wait TE will not be blocked by SODI
...
{
#ifndef MTK_FB_CMDQ_DISABLE
	dprec_logger_start(DPREC_LOGGER_PRIMARY_CMDQ_FLUSH, blocking, (unsigned int)(unsigned long)callback);
	if(blocking)
	{
...
//start 2015.07.10 lijianbin@yulong.com for lcd flick
    if (primary_display_is_video_mode()==1)
    {
        cmdqRecClearEventToken(cmdq_handle, CMDQ_EVENT_MUTEX0_STREAM_EOF);
    }
//end 2015.07.10 lijianbin@yulong.com for lcd flick
...
static int _DL_switch_to_DC_fast(void)
{
	...
	dpmgr_modify_path(pgc->dpmgr_handle, DDP_SCENARIO_PRIMARY_RDMA0_DISP, pgc->cmdq_handle_config, primary_display_is_video_mode()?DDP_VIDEO_MODE:DDP_CMD_MODE);
#else
...
#ifdef MTK_DISP_IDLE_LP	
	if(atomic_read(&isDdp_Idle)==1) rdma_config.isidle = 1;
	else 		                    rdma_config.isidle = 0;
#endif
...
static int _DC_switch_to_DL_fast(void)
{
    ...
    dpmgr_path_deinit(pgc->ovl2mem_path_handle, (int)(unsigned long)pgc->cmdq_handle_ovl1to2_config);
...
//end 2015.07.10 lijianbin@yulong.com for lcd flick
	...
	dpmgr_modify_path(pgc->dpmgr_handle, DDP_SCENARIO_PRIMARY_DISP, pgc->cmdq_handle_config, primary_display_is_video_mode()?DDP_VIDEO_MODE:DDP_CMD_MODE);
...
static void copy_lklogo_to_dc_buf(void)
{
	...
	for(line=0 ; line<primary_display_get_height() ; line++)
	{
		...
		s = (int *)( pgc->framebuffer_va + line * ALIGN_TO(primary_display_get_width(),MTK_FB_ALIGNMENT) *	primary_display_get_bpp() /8);
...
#ifdef CONFIG_FOR_SOURCE_PQ
	pgc->dpmgr_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_RDMA0_DISP, pgc->cmdq_handle_config);
#else
...
static int __build_path_decouple(void)
{
	...
	Before turn on all the m4u of diplay modules,
	...
	*/
	...
	if (ret)
	{
		printk("build path rdma to dsi fail\n");
		return ret;
	}
	...
	*/
	...
	if (ret)
	{
		printk("build path ovl to wdma fail\n");
		return ret;
	}
...
static int _convert_disp_input_to_ovl(OVL_CONFIG_STRUCT *dst, disp_input_config *src)
{
	...
	ret = disp_fmt_to_hw_fmt(src->src_fmt, (unsigned int*)(&(dst->fmt)), (unsigned int*)(&Bpp), (unsigned int*)(&bpp));
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
    ...
    if(primary_display_is_video_mode()==1 && primary_display_is_decouple_mode()==0 && gEnableOVLStatusCheck==1)
    {
	...
	if(_should_insert_wait_frame_done_token()&& (!_is_decouple_mode(pgc->session_mode)))
	{
        ...
        if(primary_display_is_video_mode()==1)//2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
		{
...
static inline bool _is_enable_esd_check(){ return IS_ESD_ENABLE; }
...
//modify for Patch ALPS02103525 by xuzhixian@yulong.com 20150604 start
#if defined(MTK_FB_SODI_SUPPORT) || !defined(CONFIG_FPGA_EARLY_PORTING)
    spm_enable_sodi(0);
#endif
//modify for Patch ALPS02103525 by xuzhixian@yulong.com 20150604 end
...
//switch to vdo mode
...
#endif
		{
			//if(_need_register_eint())
			{
				...
				//int node;
                struct device_node *node = NULL; //modify for esd check by xuzhixian@yulong.com 20150513
				int irq;
				u32 ints[2]={0, 0};
#ifdef GPIO_DSI_TE_PIN
#ifndef CONFIG_FPGA_EARLY_PORTING
...
#endif
				// 2.register eint
				node = of_find_compatible_node(NULL, NULL, "mediatek, DSI_TE_1-eint"); //modify for esd check by xuzhixian@yulong.com 20150513
				if(node)
				{
					//DISPMSG("node 0x%x\n", node);
					of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints)); //modify for esd check by xuzhixian@yulong.com 20150513
					//mt_gpio_set_debounce(ints[0], ints[1]);
					mt_eint_set_hw_debounce(ints[0],ints[1]);
					irq = irq_of_parse_and_map(node, 0); //modify for esd check by xuzhixian@yulong.com 20150513
...
//modify for P81 by xuzhixian@yulong.com 20150821 start
            ...
            if (primary_display_is_video_mode())
                ...
                cmdqRecWaitNoClear(handle, CMDQ_SYNC_TOKEN_STREAM_EOF);
#endif
//modify for P81 by xuzhixian@yulong.com 20150821 end
...
static void primary_display_frame_update_irq_callback(DISP_MODULE_ENUM module, unsigned int param)
{
    ...
    if(module == DISP_MODULE_RDMA0)
    {
        if (param & 0x20) //  rdma0 frame start
        {
			...
			rdma_get_address(DISP_MODULE_RDMA0, &rdma_address);
			update_frm_seq_info(pgc->session_id, rdma_address, 0, 0, FRM_RDMA0_SOF);
        }
        ...
        if (param & 0x4) //  rdma0 frame done
        {
			...
			unsigned long rdma_address;
			rdma_get_address(DISP_MODULE_RDMA0, &rdma_address);
			update_frm_seq_info(pgc->session_id, rdma_address, 0, 0, FRM_RDMA0_EOF);
        }
    }
	...
	if((module == DISP_MODULE_WDMA0 )&&( _is_decouple_mode(pgc->session_mode) == 1))
	{
	    if (param & 0x1) //wdma0 frame done
	    {
	        atomic_set(&decouple_fence_release_event, 1);
            wake_up_interruptible(&decouple_fence_release_wq);
			update_frm_seq_info(pgc->session_id, ddp_wdma_get_cur_addr(), 0, 0, FRM_WDMA0_EOF);
	    }
	}
#endif
...
//end 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
    ...
    if(primary_display_is_video_mode()==1 && primary_display_is_decouple_mode()==0 && gEnableOVLStatusCheck==1)
    {
       ...
       if(((ovl_status[0]&0x1)!=OVL_STATUS_IDLE) ||
          ((ovl_get_status()!=DDP_OVL1_STATUS_SUB && (ovl_status[1]&0x1)!=OVL_STATUS_IDLE)) )
       {
    ...
    if(gEnableDSIStateCheck==1)
    {
        for (i = 0; i<10; i++)
        {
            ...
            if((dsi_state[i]&0x2)==0x2)
            {
                ...
                cmdqBackupReadSlot(pgc->rdma_state_info, i*5+0, &rdma_state[i*5+0]);
                cmdqBackupReadSlot(pgc->rdma_state_info, i*5+1, &rdma_state[i*5+1]);
                cmdqBackupReadSlot(pgc->rdma_state_info, i*5+2, &rdma_state[i*5+2]);
                cmdqBackupReadSlot(pgc->rdma_state_info, i*5+3, &rdma_state[i*5+3]);
                cmdqBackupReadSlot(pgc->rdma_state_info, i*5+4, &rdma_state[i*5+4]);
                DISPERR("disp RDMA_STATE: %d, irq=0x%x, in_p_cnt=%d, in_l_cnt=%d, out_p_cnt=%d, out_l_cnt=%d \n", i, rdma_state[i*5+0], rdma_state[i*5+1], rdma_state[i*5+2], rdma_state[i*5+3], rdma_state[i*5+4]);
            }
            else
            {
...
//end 2015.05.28 lijianbin@yulong.com  modify for patch ALPS02088016
    ...
    if (_is_decouple_mode(pgc->session_mode) && (userdata == DISP_SESSION_DECOUPLE_MODE || userdata == 5))
    {
...
static int decouple_fence_release_kthread(void *data)
{
    ...
    for (;;)
    {
        wait_event_interruptible(decouple_fence_release_wq, atomic_read(&decouple_fence_release_event));
        atomic_set(&decouple_fence_release_event, 0);

        /* async callback,need to check if it is still decouple*/
        _primary_path_lock(__func__);
        if(pgc->session_mode  == DISP_SESSION_DECOUPLE_MIRROR_MODE && pgc->state != DISP_SLEEPED)
        {
            static cmdqRecHandle cmdq_handle = NULL;
			unsigned int rdma_pitch_sec, rdma_fmt;
            if(cmdq_handle == NULL)
            ...
            if (ret == 0){
               ...
               _cmdq_set_config_handle_dirty_mira(cmdq_handle);
                ...
                MMProfileLogEx(ddp_mmp_get_events()->primary_rdma_config, MMProfileFlagPulse, interface_fence, decouple_rdma_config.address);
...
static int _present_fence_release_worker_thread(void *data)
{
   ...
   while(1)
   {
        ...
        // release present fence in vsync callback
        {
            ...
            if(pgc->session_id==0)
            {
                MMProfileLogEx(ddp_mmp_get_events()->present_fence_release, MMProfileFlagPulse, -1, 0x4a4a4a4a);//modify for P95 by xuzhixian@yulong.com 20150821
                //DISPDBG("_get_sync_info fail in present_fence_release thread \n");
                continue;
            }
            ...
            if(layer_info == NULL)
            {
                MMProfileLogEx(ddp_mmp_get_events()->present_fence_release, MMProfileFlagPulse, -1, 0x5a5a5a5a);//modify for P95 by xuzhixian@yulong.com 20150821
                //DISPERR("_get_sync_info fail in present_fence_release thread \n");
                continue;
            }
...
int primary_display_capture_framebuffer_wdma(void * data)
{
	...
	ret = m4u_alloc_mva(m4uClient,M4U_PORT_DISP_WDMA0, (unsigned long)va[0], NULL,buffer_size,M4U_PROT_READ | M4U_PROT_WRITE,0,(unsigned int*)&mva[0]);
	if(ret !=0 )
	{
	...
	ret = m4u_alloc_mva(m4uClient,M4U_PORT_DISP_WDMA0, (unsigned long)va[1], NULL,buffer_size,M4U_PROT_READ | M4U_PROT_WRITE,0,(unsigned int*)&mva[1]);
	if(ret !=0 )
	{
    ...
    if(primary_display_cmdq_enabled())
	{
        ...
        while(primary_dump_wdma)
        {
            ...
            ret = m4u_cache_sync(m4uClient, M4U_PORT_DISP_WDMA0, (unsigned long)va[buf_idx], buffer_size, mva[buf_idx], M4U_CACHE_FLUSH_BY_RANGE);
    		_primary_path_lock(__func__);
    		pconfig = dpmgr_path_get_last_config(pgc->dpmgr_handle);
    		pconfig->wdma_dirty 					= 1;
...
unsigned long get_dim_layer_mva_addr(void)
{
    if(dim_layer_mva == 0){
        ...
        memset((void*)dim_layer_va, 0, frame_buffer_size);
...
int primary_display_wait_for_vsync(void *config)
{
	...
	{
		DISPDBG("VSYNC DISP_SLEEPED Return\n");
	...
	if(ret != 0)
	{
	    DISPCHECK("vsync signaled by unknow signal ret=%d\n", ret);
	}
...
int primary_suspend_clr_ovl_config(void)
{
    ...
    is_hwc_update_frame = 1; //modify for Patch ALPS02103525 by xuzhixian@yulong.com 20150604
...
//start 2015.06.05 lijianbin@yulong.com modify for mipi clk sink to lcd vddi
    if((pgc->plcm)&&(pgc->plcm->params)&&(pgc->plcm->drv)&&(pgc->plcm->drv->vddi_power_off))
    {
        pgc->plcm->drv->vddi_power_off();
    }
//end 2015.06.05 lijianbin@yulong.com modify for mipi clk sink to lcd vddi
...
#endif
//start 2015.06.05 lijianbin@yulong.com modify for mipi clk sink to lcd vddi
    if((pgc->plcm)&&(pgc->plcm->params)&&(pgc->plcm->drv)&&(pgc->plcm->drv->vddi_power_on))
    {
        pgc->plcm->drv->vddi_power_on();
    }
//end 2015.06.05 lijianbin@yulong.com modify for mipi clk sink to lcd vddi
...
static bool is_multipass_trigger = 0;
...
int primary_display_trigger(int blocking, void *callback, unsigned int userdata)
{
    ...
    else if(pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE)
    {
        ...
        _trigger_ovl_to_memory_mirror(pgc->ovl2mem_path_handle,pgc->cmdq_handle_ovl1to2_config,
        		(fence_release_callback)_olv_wdma_fence_release_callback, DISP_SESSION_DECOUPLE_MIRROR_MODE);
    }
    else if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE)
    {
        if (pgc->dc_type == DISP_OUTPUT_DECOUPLE)
        {
            ...
            MMProfileLogEx(ddp_mmp_get_events()->primary_wdma_config, MMProfileFlagPulse, pgc->dc_buf_id, writing_mva);
            DISPMSG("%s, config wdma address 0x%lx\n", __func__, decouple_wdma_config.dstAddress);
            /*
            ...
            */
            ...
            else
            {
                ...
                {
                    _trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
                                            (fence_release_callback)_ovl_fence_release_callback, DISP_SESSION_DECOUPLE_MODE, 1);
                    _ovl_fence_release_callback(DISP_SESSION_DECOUPLE_MODE);
                }
                else
                {
                    _trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
                                            (fence_release_callback)_ovl_fence_release_callback, DISP_SESSION_DECOUPLE_MODE, 0);
                }
            }
        }
...
int primary_display_memory_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (pgc->session_mode == DISP_SESSION_DECOUPLE_MODE || pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE)
	{
		...
		{
			_trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
									(fence_release_callback)_ovl_ext_fence_release_callback, DISP_SESSION_MEMORY, 1);
			_ovl_ext_fence_release_callback(DISP_SESSION_MEMORY);
		}
		else
		{
			_trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
									(fence_release_callback)_ovl_ext_fence_release_callback, DISP_SESSION_MEMORY, 0);
		}
	}
	else
	{
...
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
int primary_suspend_outputbuf_fence_release()
{
	int layer = 0;
	layer = disp_sync_get_output_timeline_id();
	mtkfb_release_layer_fence(primary_session_id, layer);
...
int primary_display_config_output(disp_mem_output_config* output, unsigned int session_id)
{
	...
	if (pgc->state == DISP_SLEEPED && DISP_SESSION_TYPE(session_id) != DISP_SESSION_MEMORY){
 ...
 /*=== create new data_config for ovl input ===*/
	...
	for(i=0; i<session_input->config_layer_num; i++) {
		...
		if(	input_cfg->buffer_source == DISP_BUFFER_ALPHA
			...
			|| cur_fence == -1 
			|| DISP_SESSION_TYPE(session_input->session_id) == DISP_SESSION_MEMORY)
...
static int _config_rdma_input(disp_session_input_config *session_input, disp_path_handle *handle)
{
	...
	ret = _convert_disp_input_to_rdma(&(data_config->rdma_config), (disp_input_config*)session_input);
	data_config->rdma_dirty= 1;
...
int primary_display_config_input_multiple(disp_session_input_config *session_input)
{
    ...
    if (primary_get_state() == DISP_SLEEPED && DISP_SESSION_TYPE(session_input->session_id) != DISP_SESSION_MEMORY){//modify for P81 by xuzhixian@yulong.com 20150821
		DISPMSG("%s, skip because primary dipslay is sleep\n", __func__);
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
...
#if 0//CONFIG_FOR_SOURCE_PQ
				if (primary_display_is_decouple_mode()) {
					dpmgr_path_trigger(pgc->ovl2mem_path_handle, handle, CMDQ_ENABLE);
					cmdqRecFlushAsync(handle);
				} else {
					_cmdq_set_config_handle_dirty_mira(handle);
...
{
	if(handle && (handle == pgc->cmdq_handle_ovl1to2_config))
	{
		return 1;
	}
	return 0;
    ...
    if(primary_display_is_video_mode())
   	{
		...
		dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_backlight, DDP_BACK_LIGHT, (unsigned long*)&level);
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				disp_lcm_set_cmd(pgc->plcm, (void*) cmdq_handle_cmd,lcm_cmd,cmd_num);
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, unsigned int format)
{
    ...
    disp_sw_mutex_lock(&(pgc->capture_lock));
    //modify for patch ALPS02124837 by xuzhixian@yulong.com 20150618 start
    _primary_path_lock(__func__);
    ...
    //modify for patch ALPS02124837 by xuzhixian@yulong.com 20150618 end
	...
	ret = m4u_alloc_mva(m4uClient,M4U_PORT_DISP_WDMA0, pbuf, NULL,buffer_size,M4U_PROT_READ | M4U_PROT_WRITE,0, (unsigned int*)&mva);
	if(ret !=0 )
	{
...
out:
    ...
    _primary_path_unlock(__func__); //modify for patch ALPS02124837 by xuzhixian@yulong.com 20150618
	disp_sw_mutex_unlock(&(pgc->capture_lock));
	DISPMSG("primary capture: end\n");
...
//	if (_get_init_setting("M4U"))
	...
	if(1)
	{
		...
		*mva = pa_start & 0xffffffffULL;
		ret = m4u_alloc_mva(client, M4U_PORT_DISP_OVL0, 0, sg_table, (pa_end - pa_start + 1), M4U_PROT_READ |M4U_PROT_WRITE, M4U_FLAGS_FIX_MVA, (unsigned int*)mva);
...
void primary_display_reset_ovl_by_cmdq(unsigned int force)
{
    ...
    if(force==0)  // warm reset ovl
    {
        ...
        if(ovl_get_status()==DDP_OVL1_STATUS_PRIMARY)
        {
            DISP_REG_SET(pgc->cmdq_handle_config, DISP_REG_OVL_RST+DISP_OVL_INDEX_OFFSET, 0x1);     
            DISP_REG_SET(pgc->cmdq_handle_config, DISP_REG_OVL_RST+DISP_OVL_INDEX_OFFSET, 0x0);
            cmdqRecPoll(pgc->cmdq_handle_config, disp_addr_convert(DISP_REG_OVL_STA+DISP_OVL_INDEX_OFFSET), 0, 0x1);
        }
    }
    else if(force==1)  // force reset ovl
    {
        ...
        if(ovl_get_status()==DDP_OVL1_STATUS_PRIMARY)
        {
            DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST, DISP_REG_OVL_RST+DISP_OVL_INDEX_OFFSET, 0x1);     
            DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST, DISP_REG_OVL_RST+DISP_OVL_INDEX_OFFSET, 0x0);
            cmdqRecPoll(pgc->cmdq_handle_config, disp_addr_convert(DISP_REG_OVL_STA+DISP_OVL_INDEX_OFFSET), 0, 0x1);
        }
    }
}
