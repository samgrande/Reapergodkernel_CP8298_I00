    ...
    )
{
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(REQ, INFO, ("Fail in set BSSID list scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    *pu4SetInfoLen = 0;
    ...
    if(u4SetBufferLen != sizeof(PARAM_SCAN_REQUEST_EXT_T)) {
		DBGLOG(REQ, INFO, ("u4SetBufferLen != sizeof(PARAM_SCAN_REQUEST_EXT_T)\n"));
        return WLAN_STATUS_INVALID_LENGTH;
    }

    if (prAdapter->fgIsRadioOff) {
        DBGLOG(REQ, INFO, ("Return from BSSID list scan! (radio off). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
        return WLAN_STATUS_SUCCESS;
    }
...
//#if CFG_SUPPORT_WFD
		...
		if ((prAdapter->rWifiVar.prP2pFsmInfo->rWfdConfigureSettings.ucWfdEnable) &&
			 ...
			 if(prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_P2P_INDEX].eConnectionState == PARAM_MEDIA_STATE_CONNECTED ) {
				DBGLOG(INIT, TRACE, ("Twice the Scan Time for WFD\n"));
			 	ucScanTime*=2;
			 	}
			}
    ...
    if (prAdapter->prGlueInfo->rRegInfo.u4RddTestMode) {
        if((prAdapter->fgEnOnlineScan == TRUE) && (prAdapter->ucRddStatus)){
            if (kalGetMediaStateIndicated(prAdapter->prGlueInfo) != PARAM_MEDIA_STATE_CONNECTED){
                ...
                /* reject the scan request */
                //shangfei@yulong.com add to stop wifi roaming 20150918 start
                cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rScanDoneTimer);
                //shangfei@yulong.com add to stop wifi roaming 20150918 end
                rStatus = WLAN_STATUS_FAILURE;
            }
        }
        else {
    ...
    )
{
	...
	if (pParamConn->pucBssid) {
		if (!EQUAL_MAC_ADDR(aucZeroMacAddr, pParamConn->pucBssid) &&
			...
			DBGLOG(INIT, TRACE, ("wrong bssid "MACSTR"to connect\n", MAC2STR(pParamConn->pucBssid)));
		}
	} else
	...
	DBGLOG(INIT, TRACE, ("ssid %s, bssid "MACSTR", conn policy %d, disc reason %d\n",
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_KEY_INDEX);
    ...
    if (u4KeyId & IS_TRANSMIT_KEY) {
        ...
        DBGLOG(REQ, ERROR, ("Invalid WEP key index: 0x%08x\n", u4KeyId));
        return WLAN_STATUS_INVALID_DATA;
    }
    ...
    )
{
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    DBGLOG(REQ, TRACE, ("Length    : 0x%08x\n", prNewKey->u4Length));
    DBGLOG(REQ, TRACE, ("Key Index : 0x%08x\n", prNewKey->u4KeyIndex));
    DBGLOG(REQ, TRACE, ("Key Length: 0x%08x\n", prNewKey->u4KeyLength));
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_REMOVE_KEY_T);
    ...
    DBGLOG(REQ, INFO, ("Length    : 0x%08x\n", prRemovedKey->u4Length));
    DBGLOG(REQ, INFO, ("Key Index : 0x%08x\n", prRemovedKey->u4KeyIndex));
    ...
    for (i = 0; i < prPmkid->u4BSSIDInfoCount; i++) {
        ...
        if (j < CFG_MAX_PMKID_CACHE) {
            ...
            DBGLOG(RSN, TRACE, ("Add BSSID "MACSTR" idx=%d PMKID value "MACSTR"\n",
                MAC2STR(prAisSpecBssInfo->arPmkidCache[j].rBssidInfo.arBSSID), (UINT_32)j,
...
//            CMD_ACCESS_REG rCmdAccessReg;
            ...
            if (u2SubId == 0x0123) {
                ...
                DBGLOG(P2P, INFO, ("Disable Hotspot Optimization!\n"));
                if (prGlueInfo->prP2PInfo)
                    prGlueInfo->prP2PInfo->fgEnableHotspotOptimization = FALSE;
                ...
                wlanoidSendSetQueryP2PCmd(prAdapter,
                        ...
                        (PUINT_8)&arHotspotOptimizationCfg,
                        NULL,
                        0
                        );
            }
            else if (u2SubId == 0x1241) {
                DBGLOG(P2P, INFO, ("Enable Hotspot Optimization!\n"));
                if (prGlueInfo->prP2PInfo)
                    prGlueInfo->prP2PInfo->fgEnableHotspotOptimization = TRUE;
    ...
    *pu4SetInfoLen = 0;
    ...
    for ( i = 0 ; i < prNetworkAddressList->u4AddressCount ; i++) {
        ...
        prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) (prNetworkAddress +
            (ULONG) (prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress)));
    }
    ...
    if (prAdapter->fgEnArpFilter) {
        ...
        DBGLOG(REQ, INFO, ("u4IpAddressCount (%u)\n", u4IpAddressCount));
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_P2P_SET_STRUC_T);
    ...
    if (prSetP2P->u4Enable) {
		...
		DBGLOG(P2P, INFO, ("prAdapter->fgIsP2PRegistered = %d\n", prAdapter->fgIsP2PRegistered));
    ...
    )
{
    ...
    *pu4WritenLen = 0;
	...
	if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
        ...
        tokens = kalSScanf(head, "SCANFREQ=%d MSCAN=%d BESTN=%d",
            &scanfreq, &mscan, &bestn);
        if (tokens != 3) {
        ...
        while ((p2 = kalStrSep((char **)&p, ",")) != NULL) {
            ...
            if (*p2 == '\0')
                continue;
            if (*p2 == 'A') {
                rCmdBatchReq.ucChannelType = rCmdBatchReq.ucChannelType == SCAN_CHANNEL_2G4 ? SCAN_CHANNEL_FULL : SCAN_CHANNEL_5G;
            } else if (*p2 == 'B') {
                rCmdBatchReq.ucChannelType = rCmdBatchReq.ucChannelType == SCAN_CHANNEL_5G ? SCAN_CHANNEL_FULL : SCAN_CHANNEL_2G4;
            } else {
        ...
        DBGLOG(SCN, TRACE, ("[BATCH] SCANFREQ=%u MSCAN=%u BESTN=%u RTT=%u\n", 
            ...
            (UINT_32)rCmdBatchReq.ucBestn, 
            (UINT_32)rCmdBatchReq.ucRtt));
    ...
    )
{
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(REQ, WARN, ("Fail in set scheduled scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    *pu4SetInfoLen = 0;
    ...
    if(scnFsmSchedScanRequest(prAdapter,
                ...
                prSchedScanRequest->u2ScanInterval) == TRUE) {
        return WLAN_STATUS_PENDING;
    }
    else {
        return WLAN_STATUS_FAILURE;
    ...
    )
{
    ...
    DBGLOG(REQ, TRACE, ("wlanoidSetGSCNAction\n"));

    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(REQ, WARN, ("Fail in set Periodically Scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    ...
    if(prCmdPscnAction->ucPscanAct == ENABLE){
        ...
        if(scnFsmPSCNAction(prAdapter,
                (UINT_8)(prCmdPscnAction->ucPscanAct)) == TRUE) {
                
            DBGLOG(REQ, INFO, ("wlanoidSetGSCNAction < --- \n"));                
            return WLAN_STATUS_PENDING;
        }
        else {
            DBGLOG(REQ, INFO, ("wlanoidSetGSCNAction < --- \n"));          
        ...
        DBGLOG(REQ, TRACE, ("set new PCSN  \n"));        
            scnCombineParamsIntoPSCN(
            prAdapter,
            NULL,
            NULL,
            NULL,
        ...
        DBGLOG(REQ, INFO, ("ENABLE or disable PCSN  \n")); 

        if(!prScanInfo->fgPscnOnnning){
            DBGLOG(REQ, INFO, ("ENABLE PCSN  \n"));            
            scnFsmPSCNAction(prAdapter, ENABLE);     
        }
        else{
            DBGLOG(REQ, INFO, ("All PCSN is disabled...  \n")); 
        }
#endif
    ...
    )
{
    ...
    DBGLOG(REQ, INFO, ("wlanoidSetGSCNAParam v1 \n"));    

    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(REQ, WARN, ("Fail in set Periodically Scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
        DBGLOG(REQ, INFO, ("Fail in set Periodically Scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff)); 
        return WLAN_STATUS_ADAPTER_NOT_READY;
    }
    if(u4SetBufferLen != sizeof(PARAM_WIFI_GSCAN_CMD_PARAMS)) {

        DBGLOG(REQ, INFO, ("(u4SetBufferLen != sizeof(P_PARAM_WIFI_GSCAN_CMD_PARAMS))\n" ));        
        return WLAN_STATUS_INVALID_LENGTH;
    }
    else if(pvSetBuffer == NULL) {
        DBGLOG(REQ, INFO, ("(pvSetBuffer == NULL)\n" ));         
        return WLAN_STATUS_INVALID_DATA;
    }
    if (prAdapter->fgIsRadioOff) {
        DBGLOG(REQ, WARN, ("Return from BSSID list scan! (radio off). ACPI=D%d, Radio=%d\n",
    ...
    //KC-XXX memcpy(prCmdGscnParam, (P_PARAM_WIFI_GSCAN_CMD_PARAMS)pvSetBuffer, sizeof(PARAM_WIFI_GSCAN_CMD_PARAMS) );
    printk("prCmdGscnParam : base_period[%u], max_ap_per_scan[%u] num_buckets[%u], report_threshold[%u]  \n",
            prCmdGscnParam->base_period, prCmdGscnParam->max_ap_per_scan, prCmdGscnParam->num_buckets, prCmdGscnParam->report_threshold);
#if 0
    for(i = 0; i < prCmdGscnParam->num_buckets; i++){
        
        printk("prCmdGscnParam->buckets : band[%u], bucket[%u] num_buckets[%u], period[%u] report_events[%u] \n",
            prCmdGscnParam->buckets[i].band, prCmdGscnParam->buckets[i].bucket, prCmdGscnParam->buckets[i].num_channels, prCmdGscnParam->buckets[i].period, prCmdGscnParam->buckets[i].report_events);
        printk("prCmdGscnParam->buckets[%d] has channel: ", i );
        for(j = 0; j < prCmdGscnParam->buckets[i].num_channels; j++){           
            printk(" %d,  ",prCmdGscnParam->buckets[i].channels[j].channel);
        }                 
            printk("  \n" );
    }
#endif    
    if(scnSetGSCNParam(prAdapter,
                prCmdGscnParam) == TRUE) {
        DBGLOG(REQ, INFO, ("wlanoidSetGSCNAParam --->scnSetGSCNParam \n"));               
        return WLAN_STATUS_PENDING;
    }
    else {
        return WLAN_STATUS_FAILURE;
    ...
    )
{
    ...
    DBGLOG(REQ, INFO, ("wlanoidSetGSCNAConfig v1 \n"));    
    ...
    if(u4SetBufferLen != sizeof(PARAM_WIFI_GSCAN_CMD_PARAMS)) {
        ...
        DBGLOG(REQ, INFO, ("(u4SetBufferLen != sizeof(CMD_GSCN_SCN_COFIG_T))\n" ));        
        return WLAN_STATUS_INVALID_LENGTH;
    }
    else if(pvSetBuffer == NULL) {
    ...
    DBGLOG(REQ, INFO, ("prCmdGscnScnConfigParam = (P_PARAM_WIFI_GSCAN_CMD_PARAMS)pvSetBuffer    \n"));
    prCmdGscnScnConfigParam = (P_PARAM_WIFI_GSCAN_CMD_PARAMS)pvSetBuffer;
    memcpy(prCmdGscnScnConfigParam, (P_PARAM_WIFI_GSCAN_CMD_PARAMS)pvSetBuffer, sizeof(P_PARAM_WIFI_GSCAN_CMD_PARAMS) );
    DBGLOG(REQ, INFO, ("prCmdGscnScnConfigParam assign  prCmdGscnScnConfig \n"));
    ...
    DBGLOG(REQ, INFO, (" report_threshold %d report_threshold %d  num_scans %d \n",
        rCmdGscnScnConfig.u4BufferThreshold, 
        rCmdGscnScnConfig.ucNumApPerScn, 
        rCmdGscnScnConfig.u4NumScnToCache  
        ));    
    if(scnFsmSetGSCNConfig(prAdapter,
                &rCmdGscnScnConfig) == TRUE) {  
        DBGLOG(REQ, INFO, ("wlanoidSetGSCNAParam --->scnSetGSCNParam \n"));               
        return WLAN_STATUS_PENDING;
    }
    else {
    ...
    )
{
    ...
    DBGLOG(REQ, INFO, ("wlanoidGetGSCNResult v1 \n"));    
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(REQ, WARN, ("Fail in set Periodically Scan! (Adapter not ready). ACPI=D%d, Radio=%d\n",
                    prAdapter->rAcpiState, prAdapter->fgIsRadioOff));
    ...
    if(scnFsmGetGSCNResult(prAdapter,
                &rGetGscnScnResultCmd) == TRUE) {
    
        DBGLOG(REQ, INFO, ("wlanoidGetGSCNResult --->scnFsmGetGSCNResult \n"));
    ...
    )
{
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    kalMemCopy(&(prHS20Info->rRCInfo),
                prRCInfo,
                sizeof(PARAM_HS20_ROAMING_CONSORTIUM_INFO));
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(CMD_WFD_DEBUG_MODE_INFO_T);
    ...
    DBGLOG(REQ,
        INFO,
        ("New WFD Debug: %d mode and period=0x%x\n", prCmdWfdDebugModeInfo->ucDebugMode,prCmdWfdDebugModeInfo->u2PeriodInteval));

    printk("New WFD Debug: %d mode and period=0x%x\n", prCmdWfdDebugModeInfo->ucDebugMode,prCmdWfdDebugModeInfo->u2PeriodInteval);
    prAdapter->rWifiVar.prP2pFsmInfo->rWfdDebugSetting.ucWfdDebugMode		= (UINT_8) 	prCmdWfdDebugModeInfo->ucDebugMode;
    prAdapter->rWifiVar.prP2pFsmInfo->rWfdDebugSetting.u2WfdSNShowPeiroid	= (UINT_16) prCmdWfdDebugModeInfo->u2PeriodInteval;
...
{
    CMD_RLM_BCN_INFO_T *prCmdBcnInfo;


    DEBUGFUNC("wlanoidSetBcnInfo");
    DBGLOG(INIT, LOUD, ("\n"));

    ASSERT(prAdapter);
    ASSERT(pu4SetInfoLen);

    *pu4SetInfoLen = sizeof(CMD_RLM_BCN_INFO_T);

    if (u4SetBufferLen < sizeof(CMD_RLM_BCN_INFO_T)) {
        return WLAN_STATUS_INVALID_LENGTH;
    }

    ASSERT(pvSetBuffer);
    prCmdBcnInfo = (CMD_RLM_BCN_INFO_T *)pvSetBuffer;

	DBGLOG(INIT, INFO, ("<tar_cmd> command = %u %u %u\n",
		prCmdBcnInfo->u4Version,
		prCmdBcnInfo->u4BcnTT_NumOfContLostThreshold,
		prCmdBcnInfo->u4BcnTT_fgKeepAwakeInLastStage));

    return wlanSendSetQueryCmd(prAdapter,
            CMD_ID_BEACON_CONFIG,
            TRUE,
            FALSE,
            TRUE,
            nicCmdEventSetCommon,
            nicOidCmdTimeoutCommon,
            sizeof(CMD_RLM_BCN_INFO_T),
            (PUINT_8)prCmdBcnInfo,
            NULL,
            0
            );
}
