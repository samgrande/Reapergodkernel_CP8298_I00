...
static int pa122_run_fast_calibration(struct i2c_client *client)
{
  ...
  if (xtalk_temp >= (data->crosstalk) && xtalk_temp < PA12_PS_OFFSET_MAX )  //wangyufei@yulong.com modify for alsps fast calibrate at 20150202
	{ 	
...
static int pa122_yulong_calibration(struct i2c_client *client,unsigned long cal_arg)
{
	...
	int ret;
	u16 sum_of_pdata = 0;
    	u8 temp_pdata[10],buftemp[2],cfg0data = 0,temp_thrd,buf[NAND_FLASH_WR_RD_SIZE];
        u8 cfg2data=0;//debug by wangyufei
  	unsigned int ArySize = 10;

	//add by wangyufei begin 20140522
	int prox_sum = 0, prox_max = 0, prox_mean = 0;
...
//wangyufei@yulong.com modify for change distance begin at 20150609
    ...
    ps_threshold_high = data->crosstalk_base+85;
    ps_threshold_low = data->crosstalk_base+55;
//wangyufei@yulong.com modify for change distance end at 20150609
