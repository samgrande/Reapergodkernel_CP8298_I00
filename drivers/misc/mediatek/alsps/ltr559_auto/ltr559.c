...
//static struct wake_lock ltr558_ps_wake_lock;
...
// begin baibo@yulong.com 20150720 for compatible
...
#endif
// end baibo@yulong.com 20150720 for compatible
/*----------------------------------------------------------------------------*/
...
//wangyufei@yulong.com add for alsps calibration load  begin at 20141124
static void ltr559_yulong_load_calibration_param(void)
{

  char buf[NAND_FLASH_WR_RD_SIZE];
  printk("YLMTK %s\n", __func__);

  memset(buf, 0, sizeof(buf));
  read_acc_ps_cal_data_from_flash(0, buf, sizeof(buf));
  printk("YLMTK read ylparam value: 0x%x, 0x%x, 0x%x\n",
  buf[0], (buf[1] << 8) + buf[2], (buf[3] << 8) + buf[4]);
  if(0x01 == buf[0])
  {
      ltr559_obj->hw->ps_threshold_high = (buf[1] << 8) + buf[2];
      ltr559_obj->hw->ps_threshold_low = (buf[3] << 8) + buf[4];
      atomic_set(&ltr559_obj->ps_thd_val_high,  ltr559_obj->hw->ps_threshold_high);
      atomic_set(&ltr559_obj->ps_thd_val_low,  ltr559_obj->hw->ps_threshold_low);
      printk("read calibrate value from ylparam successed\n");
      return 0;
  }
  else
  {
    atomic_set(&ltr559_obj->ps_thd_val_high,  ltr559_obj->hw->ps_threshold_high);
    atomic_set(&ltr559_obj->ps_thd_val_low,  ltr559_obj->hw->ps_threshold_low);
    printk("the devices have not calibration, use default value\n");
    return -1;
  }


}
//wangyufei@yulong.com add for alsps calibration load  end at 20141124



static int proximity_calibrate(unsigned long cal_arg)
{
    int i = 0;
    int prox_sum = 0, prox_max = 0, prox_mean = 0,tmp=0;
    u16 ps_threshold_high, ps_threshold_low;
...
//      mutex_lock(&LTR559_mutex); wangyufei@yulong.com modify for psensor eint begin at 20141227
       testbit_ALS = test_bit(CMC_BIT_ALS, &ltr559_obj->enable) ? (1) : (0);
    if(testbit_ALS){
            ltr559_als_disable();
    }
    mdelay(200);
    ...
    {
        mdelay(15);
        data[i]= ltr559_ps_read();
        sum = sum+data[i];
        mdelay(55);
    }
//      mutex_unlock(&LTR559_mutex); wangyufei@yulong.com modify for psensor eint end at 20141227
    ...
    if(testbit_ALS){
            ltr559_als_enable(als_gainrange);
    }
    prox_mean = sum/20;
    ...
    ps_threshold_high = (u16)(prox_mean + 70);
        ps_threshold_low  = (u16)(prox_mean + 50);

//  else
//  {
//      printk(KERN_ERR "Clibrate failed!\n");
//      atomic_set(&dev->ps_cal_result, 30);
//      input_report_abs(dev->ps_input, ABS_DISTANCE, 30);
...
//  }
    ...
    atomic_set(&ltr559_obj->ps_thd_val_high,  ps_threshold_high);
    atomic_set(&ltr559_obj->ps_thd_val_low,  ps_threshold_low);
    ltr559_ps_set_thres();
    memset(buf, 0, sizeof(buf));
    ...
    write_acc_ps_cal_data_to_flash(0, buf, NAND_FLASH_WR_RD_SIZE);
    printk("%s: calibrate data is write to nand flash\n", __func__);
    printk("ps_threshold_high = 0x%x, ps_threshold_low = 0x%x, prox_mean = 0x%x\n",
        ps_threshold_high, ps_threshold_low, prox_mean);
#endif
    //printk("in %s: after ps_detected_thres = 0x%x, ps_undetected_thres = 0x%x\n",
    //  __func__, ltr558_gdata->ps_detected_thres, ltr558_gdata->ps_undetected_thres);
    
//write prox_threshold_hi and prox_threshold_lo to nand flash to save
//  input_report_abs(dev->ps_input,ABS_DISTANCE,20);
//  input_sync(dev->ps_input);
    //atomic_set(&dev->ps_cal_result, 20);
    return 0;
}
/* add by mayulong 20121203 end */
/* add by mayulong 20130502 begin */
static int ltr559_proximity_fast_calibrate()
{
    int i = 0;
    u16 prox_sum = 0, prox_max = 0, prox_mean = 0;
    u16 ps_threshold_high, ps_threshold_low;
    ...
    printk("YULONGMTK in %s: fast prox_mean overage is %d\n", __func__, prox_mean);
...
//  }
    ...
    if(prox_mean < 600)
    {
        ...
        atomic_set(&ltr559_obj->ps_thd_val_high, ps_threshold_high);
        atomic_set(&ltr559_obj->ps_thd_val_low, ps_threshold_low);
        printk("fast calibrate ok, use fast calibrate value LTR559_PS_THRES_LOW_0 = %d, LTR559_PS_THRES_UP_0 = %d\n",
                                ps_threshold_low,   ps_threshold_high);
        ltr559_ps_set_thres();
    }
    else if((prox_mean >= 600) && (0x01 == buf[0]))
    {
        
        atomic_set(&ltr559_obj->ps_thd_val_high, (u16)(((buf[1] << 8) & 0xff00) + (buf[2] & 0xff)));
        atomic_set(&ltr559_obj->ps_thd_val_low, (u16)(((buf[3] << 8) & 0xff00) + (buf[4] & 0xff)));
        printk("fast calibrate fail, use YL_PARAME calibrate value ltr559_ps_threshold_low = %d, ltr559_ps_threshold_high = %d\n",
    ...
    else
    {
        atomic_set(&ltr559_obj->ps_thd_val_high,  ltr559_obj->hw->ps_threshold_high);
        atomic_set(&ltr559_obj->ps_thd_val_low,  ltr559_obj->hw->ps_threshold_low);
        printk("fast calibrate fail, use default calibrate value ltr559_ps_threshold_low = %d, ltr559_ps_threshold_high = %d\n",
                                ltr559_obj->hw->ps_threshold_low,   ltr559_obj->hw->ps_threshold_high);
        ltr559_ps_set_thres();
    }
    
    return 0;
}
/* add by mayulong 20130502 end */
...
static int ltr559_i2c_read_reg(u8 regnum)
{
    ...
    int res = 0;
    //wangyufei@yulong.com modify for psensor eint begin at 20141227
    buffer[0]= regnum;
    ...
    mutex_unlock(&LTR559_mutex);
    //wangyufei@yulong.com modify for psensor eint end at 20141227
    if(res <= 0)
    {
...
static ssize_t ltr559_store_status(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    {
        //APS_DBG("invalid content: %d, length = %d\n", (int)buf, count);
    }
    return count;    
}
...
static ssize_t ltr559_store_reg(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(2 == sscanf(buf, "%hhu %x ", &reg,&value))
    { 
...
static int ltr559_ps_set_thres(void)
{
    ...
    else
    {
        ...
        printk("use YL_PARAME calibrate value ltr559_ps_threshold_low = %d, ltr559_ps_threshold_high = %d\n",
...
static int ltr559_ps_enable(int gainrange)
{
     ...
     */
//wangyufei@yulong.com add for alsps range increase begin at 20141124
  error = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 12);
    if(error<0)
    {
        APS_LOG("ltr559_ps_enable() error ltr559_ps_n_pulses\n");
        return error;
    } 

  error = ltr559_i2c_write_reg(LTR559_PS_LED, 0x9F);
    if(error<0)
    {
        APS_LOG("ltr559_ps_enable() error LTR559_PS_LED...\n");
        return error;
    }
//wangyufei@yulong.com add for alsps range increase end at 20141124
...
static int ltr559_ps_read(void)
{
    int psval_lo, psval_hi, psdata=0;
...
/*
    error = ltr559_i2c_write_reg(LTR559_ALS_MEAS_RATE, 0x0A); 
    if(error<0)
    {
        APS_LOG("ltr559_als_enable() error0...\n");
        return error;
    }
*/
    mdelay(WAKEUP_DELAY);
...
static int ltr559_als_read(int gainrange)
{
    ...
    /*
    int tmp=0;
    tmp = ltr559_i2c_read_reg(LTR559_ALS_CONTR);
    printk("LTR559_ALS_CONTR = %d\n",tmp);

    tmp = ltr559_i2c_read_reg(LTR559_ALS_MEAS_RATE);
    printk("LTR559_ALS_CONTR = %d\n",tmp);

    tmp = ltr559_i2c_read_reg(0x86);
    printk("LTR559_ALS_CONTR = %d\n",tmp);

    tmp = ltr559_i2c_read_reg(LTR559_ALS_PS_STATUS);
    printk("LTR559_ALS_CONTR = %d\n",tmp);

    
    tmp = ltr559_i2c_read_reg(LTR559_PS_LED);
    printk("LTR559_PS_LED = %d\n",tmp);
  */
    //zhangzhao
...
//  mutex_unlock(&LTR559_mutex);wangyufei@yulong.com modify for psensor eint end at 20141227
    ...
    //printk("ratio = %d  gainrange = %d\n",ratio,gainrange);
    if (ratio < 45){
        luxdata_int = (((17743 * alsval_ch0)+(11059 * alsval_ch1))/3)/10000;//gainrange
    }
    else if ((ratio < 64) && (ratio >= 45)){
        luxdata_int = (((42785 * alsval_ch0)-(19548 * alsval_ch1))/3)/10000;//gainrange
    }
    else if ((ratio < 85) && (ratio >= 64)) {
        luxdata_int = (((5926 * alsval_ch0)+(1185 * alsval_ch1))/3)/10000;//gainrange
    }
    else {
        luxdata_int = 0;
        }
    //printk("als_value_lux = %d\n", luxdata_int);

    luxdata_int=luxdata_int*9;

    if(luxdata_int >=65535)
        {
            luxdata_int=65535;
        }
...
int ltr559_setup_eint(struct i2c_client *client)
{
    ...
    //mdelay(500);//chenbing3@yulong.com add for sys reboot would be reboot because of psensor interrupt 20150730
    mt_eint_mask(CUST_EINT_ALS_NUM);//chenbing3@yulong.com del for mask psensor interrupt 20150810
    return 0;
}
...
static int ltr559_devinit(void)
{
    ...
    /* add by mayulong 20121203 begin */
/*memset(buf, 0, sizeof(buf));   //wangyufei@yulong.com modify for alsps calibration load
     read_acc_ps_cal_data_from_flash(0, buf, sizeof(buf));
    printk("YLMTK read ylparam value: 0x%x, 0x%x, 0x%x\n", 
        buf[0], (buf[1] << 8) + buf[2], (buf[3] << 8) + buf[4]);
    if(0x01 == buf[0])
    ...
    else
        {
            ...
            atomic_set(&ltr559_obj->ps_thd_val_low,  ltr559_obj->hw->ps_threshold_low);
            printk("the devices have not calibration, use default value\n");            
   }*/                           //wangyufei@yulong.com modify for alsps calibration load
/* add by mayulong 20121203 end */

    // Enable PS to Gain4 at startup
    init_ps_gain = PS_RANGE16;
    ps_gainrange = init_ps_gain;
/*chenbing3@yulong.com modify for no interrupt in init 20150810 begin*/
#if 0
    ...
    if (res < 0)
        goto EXIT_ERR;
#endif
/*chenbing3@yulong.com modify for no interrupt in init 20150810 end*/
    ...
    {   
       ...
       printk("ltr559_setup_eint before \n");
    if((res = ltr559_setup_eint(client))!=0)
    {
        APS_ERR("setup eint: %d\n", res);
        return res;
    }
    printk("ltr559_setup_eint after \n");
...
//  u8 reg_value[1];
    ...
    else
    {
        ...
        printk("ltr559_eint_work rawdata ps=%d als_ch0=%d!\n",obj->ps,obj->als);
...
/*singal interrupt function add*/
        ...
        if(intr_flag_value){
                printk(" interrupt value ps will < ps_threshold_low ----near----");
                ...
                printk(" interrupt value ps will > ps_threshold_high ----far----");
                databuf[0] = LTR559_PS_THRES_LOW_0; 
       ...
       unsigned long arg)       
{
    ...
    switch (cmd)
    {
        ...
        case ALSPS_GET_ALS_RAW_DATA:    
            ...
            break;
//wangyufei@yulong.com add for alsps calibration load begin at 20141125
      case ALSPS_IOCTL_SET_CALI:
        printk("ltr559 yulong_load calibration enable start\n");
        ltr559_yulong_load_calibration_param();
      break;
//wangyufei@yulong.com add for alsps calibration load end at 20141125
        default:
        ...
        void* buff_out, int size_out, int* actualout)
{
    ...
    switch (command)
    {
        ...
        case SENSOR_ENABLE:
          ...
          ltr559_proximity_fast_calibrate();  //wangyufei@yulong.com add for fast_calibration at 20141124
                    set_bit(CMC_BIT_PS, &obj->enable);
                }
                else
                {
...
/* begin baibo@yulong.com 20150720 for decice info */
#ifdef SENSOR_DEVICE_INFO
extern int get_device_info(char* buf);
#endif
/* end baibo@yulong.com 20150720 for decice info */
...
/* begin baibo@yulong.com 20150720 for decice info */
#ifdef SENSOR_DEVICE_INFO
  sprintf(tembuff,"ALPS: LTR559 LRTEON\n");
  get_device_info(tembuff); 
#endif
/* end baibo@yulong.com 20150720 for decice info */
    APS_LOG("%s: OK\n", __func__);
#if defined(MTK_AUTO_DETECT_ALSPS)
    ltr559_init_flag = 0; /* begin baibo@yulong.com 20150720 for compatible */
#endif
    return 0;
...
//  MT6516_EINTIRQMask(CUST_EINT_ALS_NUM);  /*mask interrupt if fail*/
    APS_ERR("%s: err = %d\n", __func__, err);
#if defined(MTK_AUTO_DETECT_ALSPS)
    ltr559_init_flag = -1; /* begin baibo@yulong.com 20150720 for compatible */
#endif
    return err;
}
