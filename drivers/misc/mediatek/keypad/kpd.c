...
#endif
/* add for volume down key enbale by liyongfeng@yulong.com  20150430 begin */
#define GPIO_VOL_DOWN_EINT_PIN         (GPIO85 | 0x80000000)
#define GPIO_VOL_DOWN_EINT_PIN_M_GPIO   GPIO_MODE_00
#define GPIO_VOL_DOWN_EINT_PIN_M_EINT   GPIO_MODE_00
#define CUST_EINT_VOL_DOWN_NUM              85
#define CUST_EINT_VOL_DOWN_DEBOUNCE_CN      0
#define CUST_EINT_VOL_DOWN_TYPE    CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_VOL_DOWN_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE
struct work_struct work_vol_down;
static struct workqueue_struct *mtk_vol_down_key_wq;
void vol_down_eint_handler();
 static void vol_down_work_func(struct work_struct *work)
 {
   int homekey_value;
   homekey_value=mt_get_gpio_in(GPIO_VOL_DOWN_EINT_PIN);
   if(homekey_value)
   {
     mt_eint_registration(CUST_EINT_VOL_DOWN_NUM, CUST_EINTF_TRIGGER_LOW,vol_down_eint_handler, 1);
     input_report_key(kpd_input_dev, 0x72, 0);
     input_sync(kpd_input_dev);
   }
   else
   {
    mt_eint_registration(CUST_EINT_VOL_DOWN_NUM, CUST_EINTF_TRIGGER_HIGH, vol_down_eint_handler, 1);
    mt_eint_set_sens(CUST_EINT_VOL_DOWN_NUM, MT_LEVEL_SENSITIVE);
    input_report_key(kpd_input_dev, 0x72, 1);
    input_sync(kpd_input_dev);
   }
}
...
void vol_down_eint_handler()
{
    printk("vol down interrupt has been triggered\n");
    if(mt_get_gpio_in(GPIO_VOL_DOWN_EINT_PIN))
     mt_eint_registration(CUST_EINT_VOL_DOWN_NUM, 8, vol_down_eint_handler, 1);
    else
     mt_eint_registration(CUST_EINT_VOL_DOWN_NUM, 4, vol_down_eint_handler, 1);
    queue_work(mtk_vol_down_key_wq, &work_vol_down);
}

void mt_eint_register_vol()
{
       mt_set_gpio_mode(GPIO_VOL_DOWN_EINT_PIN, GPIO_VOL_DOWN_EINT_PIN_M_EINT);
       mt_set_gpio_dir(GPIO_VOL_DOWN_EINT_PIN, GPIO_DIR_IN);
       mt_set_gpio_pull_enable(GPIO_VOL_DOWN_EINT_PIN, GPIO_PULL_ENABLE);
       mt_set_gpio_pull_select(GPIO_VOL_DOWN_EINT_PIN, GPIO_PULL_UP);
       mt_eint_registration(CUST_EINT_VOL_DOWN_NUM, CUST_EINT_VOL_DOWN_TYPE, vol_down_eint_handler, 1);
       mt_eint_unmask(CUST_EINT_VOL_DOWN_NUM);
}

/* add for volume down key enbale by liyongfeng@yulong.com  20150430 end*/
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
    ...
    kpd_memory_setting();
/* add for volume down key enbale by liyongfeng@yulong.com  20150430 begin */
     INIT_WORK(&work_vol_down, vol_down_work_func);
     mtk_vol_down_key_wq = create_singlethread_workqueue("mtk_vol_down_key_wq");
    if (!mtk_vol_down_key_wq)
    {
        printk("Error Could not create work queue mtk_vol_down_key_wq: no memory");
        return 0;
    }
/* add for volume down key enbale by liyongfeng@yulong.com  20150430 end */
